...
\usepackage{fontspec}
% Set up a Devanagari-capable font without hard-failing on one local family.
...
\IfFontExistsTF{Abyssinica SIL}{
  ...
  \IfFontExistsTF{Arial Unicode MS}{
    \newfontfamily\ethiopic{Arial Unicode MS}
  }{
    \newfontfamily\ethiopic{Times New Roman}
  }
}
\IfFontExistsTF{Amiri}{
  ...
  \IfFontExistsTF{Geeza Pro}{
    \newfontfamily\persian[Script=Arabic]{Geeza Pro}
  }{
    \newfontfamily\persian{Arial Unicode MS}
  }
}
...
% Auto-generate author index from all citations
...
% Avoid a second self-citation on the impressum page: it generates duplicate
% langsci-current-book aux entries and triggers a spurious end-of-run warning.
\makeatletter
\patchcmd{\lsImpressum}{\CiteFullAuthorList{langsci-current-book}}{\@author}{}{}
\AtEndDocument{%
  \let\savepicturepage\@gobbletwo
  \let\pgfsyspdfmark\@gobblethree
}
\makeatother

% Semantic typography macros
% Make nested emphasis inside italicized examples bold italic rather than
% reverting to upright text.
\DeclareEmphSequence{\itshape,\bfseries\itshape}
