...
\begin{itemize}[noitemsep]
    ...
    \item \textit{Front (15)}: For \is{palatal!sounds}palatal sounds like /ʃ/ (the $\langle$sh$\rangle$ in \mention{ship}).
...
\begin{itemize}[noitemsep]
    \item \textit{High front}: /i/ (as in \mention{beet}), /ɪ/ (as in \mention{bit})
    \item \textit{High back}: /u/ (as in \mention{boot}), /ʊ/ (as in \mention{book})
    \item \textit{Mid front}: /ɛ/ (as in \mention{bet})
    \item \textit{Mid central}: /ə/ (as in the first syllable of \mention{about})
    \item \textit{Mid back}: /ʌ/ (as in \mention{but})
    \item \textit{Low front}: /æ/ (as in \mention{bat})
    \item \textit{Low back}: /ɑ/ (as in \mention{bot})
\end{itemize}
...
\subsection{The pronunciations of \textit{--\op e\cp s} and \mention{--ed}}

Two of the most common inflectional endings in English are \textit{--\op e\cp s}, used for plural nouns and third-person singular present tense verbs, and \mention{--ed}, used for past tense and past participle forms of regular verbs. Despite their simple spellings, these endings can be pronounced in multiple ways depending on the phonological context in which they appear.
