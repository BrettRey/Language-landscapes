...
\textbf{Uvula (9)}\quad Some languages use the \is{uvular}uvula as a place of articulation. This is uncommon in English, though it occurs in some dialects. An example of a uvular sound is the French\il{French} /ʁ/ in \textit{rouge}.
...
\begin{itemize}[noitemsep]
    ...
    \item \textit{Dental} (3): Tongue tip behind or between teeth. Examples: /θ/ (as in \mention{thin}), /ð/ (as in \mention{this})
    \item \textit{Alveolar} (4): Tongue tip or blade on the \is{alveolar!ridge}alveolar ridge. Examples: /t/, /d/, /s/, /z/, /n/, \& /l/
    \item \textit{Post-alveolar} (5): Tongue blade behind the alveolar ridge. Examples: /r/, /ʃ/ (as in \mention{ship}), /ʒ/ (as in \mention{measure}), /t͡ʃ/ (as in \mention{chip}), \& /d͡ʒ/ (as in \mention{judge}). Here, /t͡ʃ/ and /d͡ʒ/ are single phonemes, even though they're represented in the IPA with two symbols each.
    \item \textit{Palatal} (7): Tongue body against the hard palate. Example: /j/ (as in \mention{yes})
...
\textsc{\is{diphthong|(}Diphthongs} are vowel sounds that involve a change in quality during their production (from Greek \textit{diphthongos} `having two sounds'). For example, \mention{ride} starts at /a/ and ``glides'' to /ɪ/. My dialect includes the following diphthongs.
...
This is, incidentally, a useful diagnostic. If someone confidently tells you English has five vowels, you can be fairly certain they're talking about letters. If they say six, they've remembered \textit{y}. If they say twenty-odd, they may have taken a linguistics course and would like to tell you about it.
\end{tblsframed}
...
It also matters because if students sometimes perceive us as saying the \enquote{clear [l]} in \textit{love} and other times the \enquote{dark [ɫ]} in \textit{ball}, but we think they're always hearing only the English /l/, then there's a good deal of possibility for miscommunication.
...
Intonation can be hard to notice consciously~-- we tend to hear \textit{what} someone says before we hear \textit{how} they say it. But intonation errors can cause real pragmatic misfires~-- a flat or falling intonation on what's meant as a question can sound like a statement or even a command.\is{intonation|)}
