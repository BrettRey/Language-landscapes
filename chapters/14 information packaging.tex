...
\ea[\textsuperscript{?}]{\textit{It's a hybrid. I bought a new car yesterday.}}\label{ex:hybrid-reversed}
\z
...
\ea[]{\textit{Regarding the broken laptop, blame Steve.}}\label{ex:broken-laptop}
\z
...
\ea\label{ex:CNN}
    \ea[]{[\textit{A convoluted neural network \uline{architecture}]\cb~\ob \uline{is} formed by a stack of distinct layers that transform the input volume into an output volume (e.g. holding the class scores) through a differentiable function.}]\\\hfill[\textsc{passive}]}
    \ex[]{[\textit{A \uline{stack} of distinct layers that transform the input volume into an output volume (e.g. holding the class scores) through a differentiable function\cb~\ob \uline{forms} a convoluted neural network architecture.}]\hfill[\textsc{active}]}\z
\z
