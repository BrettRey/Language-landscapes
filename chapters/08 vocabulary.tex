...
\begin{itemize}[noitemsep, leftmargin=*]
    ...
    \item Analyze word structure using morphological concepts (bases, affixes, derivation, inflection)
...
Try your morphological skills: (1) Identify the base and affixes in \textit{uncomfortable}, (2) Explain why \textit{singer} and \textit{singing} belong to different lemmas but the same word family, (3) Create three new words using the productive suffix \textit{-ize}.
\end{tblsframedsymbol}
...
\begin{enumerate}[noitemsep, leftmargin=*]
    \item The words \textit{biology}, \textit{biography}, and \textit{biodegradable} all begin with \textit{bio-}.
    ...
    \item Consider the verb \textit{sing}: \textit{sing}, \textit{sings}, \textit{singing}, \textit{sang}, \textit{sung}.
    \begin{enumerate}[nosep]
    ...
    \item A student writes: \enquote{I looked it up in the dictionary but couldn't find \textit{happily}.}
...
\textbf{Word tokens}\quad A \textsc{word token} is a single instance or occurrence of a word in a text or utterance. If an article is 800 words long, it has 800 tokens, even if 56 of those tokens are \textit{the}.
...
[\texttt{family}\\\textit{wide}, align=center
 [\texttt{lemma}\\\textit{wide} (adj), align=center
   [\texttt{shape}\\\textit{wide}, align=center
     [\texttt{token}\\\textit{a \emph{wide} river}, align=center]
   ]
   [\texttt{shape}\\\textit{wider}, align=center
     [\texttt{token}\\\textit{a \emph{wider} river}, align=center]
   ]
   [\texttt{shape}\\\textit{widest}, align=center
     [\texttt{token}\\\textit{the \emph{widest} river}, align=center]
   ]
 ]
 [\texttt{lemma}\\\textit{widen} (verb), align=center
   [\texttt{shape}\\\textit{widen}, align=center
     [\texttt{token}\\\textit{to \emph{widen} the gap}, align=center]
   ]
   [\texttt{shape}\\\textit{widens}, align=center
     [\texttt{token}\\\textit{the gap \emph{widens}}, align=center]
   ]
   [\texttt{shape}\\\textit{widened}, align=center
     [\texttt{token}\\\textit{they \emph{widened} it}, align=center]
   ]
 ]
 [\texttt{lemma}\\\textit{width} (noun), align=center
   [\texttt{shape}\\\textit{width}, align=center
     [\texttt{token}\\\textit{the \emph{width} of}, align=center]
   ]
 ]
]
\end{forest}
\caption{A tree diagram illustrating the relationship between a word family, its lemmas (with lexical category), types (inflected forms), and contextual tokens. The diagram uses the \textit{wide} family as an example.}
...
Think about the implications: If \textit{the} appears 60,800 times per million words but \textit{purchase} only 90 times, what does this mean for teaching priorities? How might this frequency cliff affect your approach to vocabulary selection and sequencing?
...
\begin{enumerate}[noitemsep, leftmargin=*]
    ...
    \item \textit{The} appears about 60,800 times per million words, while \textit{purchase} appears three orders of magnitude less frequently.
    \begin{enumerate}[nosep]
...
Consider teaching implications: Why might high-frequency words like \textit{get}, \textit{take}, or \textit{make} be particularly challenging for learners? How does polysemy relate to the frequency patterns discussed earlier in this chapter?
\end{tblsfilledsymbol}
...
\begin{enumerate}[noitemsep, leftmargin=*]
    \item The lemma \textit{head} (noun) can refer to: the body part, the leader of an organization, the foam on beer, and the front of a line. Explain how these meanings might be related to the lemma \textit{head} (verb). What does this tell us about how word meanings develop over time?
...
Many instructional materials pay little attention to frequency data. Textbooks sometimes include words like \textit{archipelago} or \textit{obsequious} alongside genuinely useful vocabulary, often because they fit a thematic unit or appear on a test. But the opportunity cost is real. Even seemingly \enquote{common} words can be deceptive~-- \textit{humble} feels familiar to proficient speakers but falls outside the most frequent 5,000 words.
...
\begin{enumerate}[noitemsep, leftmargin=*]
    \item A textbook presents these words together in a unit on \enquote{emotions}: \textit{happy}, \textit{sad}, \textit{angry}, \textit{frustrated}, \textit{elated}, \textit{melancholy}.
    \begin{enumerate}[nosep]
    ...
    \item A student asks about the difference between \textit{big} and \textit{large}.
    \begin{enumerate}[nosep]
