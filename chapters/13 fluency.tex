...
\epigraph{Cotton needs motion\\
to dry to softness.}{Frances Boyle, \enquote{Pegging Out Washing}}

\begin{tblsframedsymbol}{Learning objectives}{bulb}
\begin{itemize}[noitemsep, leftmargin=*]
    \item Distinguish between fluency as flow and general language proficiency
    \item Explain the complexity-accuracy-fluency (CAF) triangle and how attention affects each dimension
    \item Identify the key characteristics of fluent vs. disfluent speech patterns
    \item Understand how vocabulary knowledge correlates with fluency across different language skills
    \item Recognize factors that promote or hinder fluency development in language learners
    \item Apply strategies for balancing fluency development with accuracy and complexity goals
    \item Appreciate the role of repairs and hesitations as natural parts of language learning
\end{itemize}
\end{tblsframedsymbol}

\begin{tblsfilled}{Analytical approaches in this chapter}
This chapter introduces several key concepts for understanding language fluency:
\begin{itemize}[noitemsep, leftmargin=*]
    \item CAF triangle analysis: Understanding how complexity, accuracy, and fluency compete for limited attentional resources
    \item Flow vs. proficiency: Distinguishing between smooth language processing and overall language knowledge
    \item Temporal measures: Using speech rate, pause patterns, and hesitation markers to assess fluency
    \item Cross-skill comparison: Examining how fluency manifests differently in speaking, listening, reading, and writing
    \item Repair analysis: Understanding how self-corrections and reformulations function in language production
\end{itemize}
These approaches help teachers recognize fluency as a multifaceted aspect of language proficiency that can be developed through targeted practice.
\end{tblsfilled}
...
Technical terms are often drawn from common words but have more or less differing meanings. For us, technically, \enquote{\term{fluency} can be described as the ability to process language receptively and productively at a reasonable speed} \citep[11]{Nation2014b}. Let's be careful about ability though. Linguistic proficiency is purely internal, but our ability to apply it runs smack into the world around us. Knowing more vocabulary, for example, is an internal, linguistic factor that will typically increase your fluency via your proficiency, but being in a noisy, distracting environment is an external factor that will typically decrease your ability to process language. In such an environment, you may become slow or halting, which is to say disfluent.
...
\begin{tblsframedsymbol}{Check your understanding: the CAF triangle}{test}
Consider your own language learning experience:
\begin{itemize}[noitemsep, leftmargin=*]
    \item When you focus intensely on getting grammar exactly right (accuracy), what happens to your speaking speed and smoothness (fluency)?
    \item When you try to use complex vocabulary or sentence structures (complexity), how does this affect your fluency?
    \item Can you think of a time when reducing pressure for perfection actually improved your communication?
\end{itemize}
This illustrates how our limited attention creates trade-offs between the three dimensions of language performance.
\end{tblsframedsymbol}

\subsection{Fluency vs proficiency}
\is{fluency!vs proficiency|(}

Forgive me for going on, but I often find that people confound fluency and proficiency, even once the distinction has been clearly set out. And this is understandable; the two are related concepts. Still, they are distinct. High linguistic proficiency makes fluency much easier over a broad range of situations, tasks, and topics, but someone with low proficiency may still be quite fluent in limited situations. Consider Table~\ref{tab:fluency_proficiency}.\footnote{For an example of low proficiency and fluency, see \url{https://youtu.be/48yr93hwvCk?t=21}. For low proficiency with much higher fluency, see \url{https://youtu.be/m0UGhSufSJk?t=45}. And for high proficiency with low fluency, see this interview of Ted Kennedy: \url{https://youtu.be/e5TkhNWPspM?t=76}.}
...
\lsptoprule
 & \emph{Fluent} & \emph{Proficient} \\
\midrule
Definition & Flowing production or processing & High ability to flow, not always actualized \\
Scope & Particular to one skill in a particular situation (e.g., reading) & General skills, grammatical and lexical knowledge \\
Consistency & Varies by occasion & Typically stable or growing \\
\lspbottomrule
\end{tabular}
...
\ea
    \ea[]{So then I said to him, I said, \enquote{Look, if you're not happy with the arrangement, we can always renegotiate. But we need to do it soon, before the project deadline.} And he just sort of nodded and said, \enquote{Yeah, okay, let's talk tomorrow.} So, I think it's going to work out.}\label{ex:fluent}
    \ex[]{So then I said to him, I said, um, \enquote{Look, if you're not, uh, happy with the, the arrangement, we can always, you know, rene\dots, renegotiate. But we need to do it, uh, soon, before the, um, project deadline.} And he just sort of, uh, nodded and said, \enquote{Yeah, okay, let's, let's talk tomorrow.} So, I, I, I think it's, uh, it's going to work out.}\label{ex:disfluent}
    \z
\z
The first example (\ref{ex:fluent}) flows smoothly, with few pauses or hesitations. The speaker seems to know exactly what they want to say and how to say it. In contrast, the second example (\ref{ex:disfluent}) has a bunch of filled pauses (\mention{um}, \mention{uh}), repeated words and phrases, and false starts (\mention{rene}--, \mention{renegotiate}). The speaker seems to be grasping for words, struggling to formulate their thoughts in real time. Not that this is bad. In fact, it's quite normal. It's just less fluent than (\ref{ex:fluent}).

But fluency isn't just about speed or smoothness. It's about \term{automaticity}, the ability to retrieve and articulate words and phrases without conscious effort. It's about \term{proceduralization}, the gradual transformation of declarative knowledge (``knowing that'') into procedural skill (``knowing how'').\is{cognitive load}

Think about the first time you tried to drive a car (if you drive). Everything required conscious effort: checking the mirrors, signalling, shifting gears, monitoring your speed. But over time, these actions became automatic, proceduralized. You could do them without thinking, freeing up cognitive resources for higher-level tasks like navigating or conversing with passengers. If you're like me, you may have set out to drive one place and instead found yourself on your regular route to work, a route you've taken so many times that it has become automatic.
...
And our main cognitive limitation is that: attention. Linguists used to be very focused on trying to imagine grammars that use as little memory as possible on the assumption that our memory for individual words and expressions was quite limited. The thinking was \enquote{why remember \mention{jump}, \mention{jumped}, \mention{jumps}, \mention{jumping} along with all the other regular verb forms when you can just remember \mention{jump} and then produce the rest on the fly with rules?} It seems more likely that memory retrieval may sometimes be a problem, but \enquote{storage capacity} is practically unlimited \citep{Erman2000, JackendoffAudring2018}.

\begin{tblsframedsymbol}{Fluency reflection: automaticity in action}{test}
Think about activities you've automatized in your life:
\begin{itemize}[noitemsep, leftmargin=*]
    \item What language tasks feel automatic to you in your first language? (greeting people, ordering coffee, giving directions?)
    \item In a second language you're learning, which elements still require conscious effort?
    \item How might reducing the cognitive load of basic language functions help improve overall communication?
\end{itemize}
Understanding automaticity helps explain why fluency activities often focus on familiar, well-practiced language patterns.
\end{tblsframedsymbol}
...
Then there's reading fluency. As discussed in~\ref{sec:simple-view-of-reading}, the Simple View of Reading \citep{Gough1986} shows that fluent reading depends on both smooth word decoding and language comprehension ($RC=D\times LC$). But in the context of fluency development, what matters is how decoding automaticity frees up attention for comprehension: when decoding requires effort, it monopolizes the limited cognitive resources needed for following narrative threads and integrating ideas.

Because English has a somewhat opaque orthography, few students are able to fully transfer their existing decoding ability from another language, even if it has a Roman-alphabetic script. That said, most people who read in a Germanic or Romance language will have a strong head start in decoding English, and anyone who is literate in a Roman-alphabetic script will also be able to transfer some skills. Those who have little or no experience reading alphabetic scripts will necessarily have low reading fluency, even if they have good listening fluency in English.
...
These fluencies, though distinct, are deeply intertwined. Progress in one often fuels progress in the others. As your spoken vocabulary grows, so does your listening comprehension. As your reading fluency develops, so does your sense of what \enquote{sounds right} in your own writing.
...
But the relationships are complex and sometimes counterintuitive. You can be a fluent speaker but a halting reader, a prodigy at writing but tongue-tied in conversation. During my time in Tokyo\il{Japanese}, I had practised a handful of Japanese phrases well enough to ask for directions with a semblance of fluency. The trouble was that the rapid, detailed, idiomatic responses far exceeded my listening fluency. I would nod and smile in feigned understanding, set off in the general direction indicated, and hope for the best. Speaking and listening are different skills, and fluency in one guarantees nothing about the other.
\is{dimensions of fluency|)}
...
A variety of tasks may be used to measure speaking fluency. Spontaneous speech tasks: Participants might be asked to describe a picture, tell a story, provide directions, or engage in a conversation based on a specific prompt. Researchers analyse the transcript afterward, looking at elements like words per minute, pauses, hesitations, and grammatical complexity.
...
The average speaking rate for native English speakers in the United States is around 150 words per minute (wpm) for general conversation.\footnote{This widely cited figure comes from the National Center for Voice and Speech; see \url{https://tfcs.baruch.cuny.edu/speaking-rate/}.} But this rate varies depending on the context and purpose of the speech. Presentations tend to be slower (100--150 wpm), while audiobooks are typically read at 150--160 wpm. Auctioneers and sports commentators can reach 250--400 wpm.
...
Along with the obvious implications for speaking, this has implications for listening. One study of recorded lecture comprehension in competent anglophones, which employed audio, slides, and video of the speaker, found no significant difference between the $1\times$ viewing speed (about 160 wpm) and $2\times$, and only minor degradation at $2.5\times$ \citep{Murphy2022}.
\is{speech rate|)}
...
The single strongest predictor of fluency across skills is vocabulary knowledge: roughly half the variance in reading fluency, a third of listening comprehension. That fact alone reframes how you allocate instructional time. And the CAF triangle gives you a way to read a student's hesitation: are they slow because they lack the words, or because they're investing attention in accuracy? The answer determines whether the right move is vocabulary building, lower-stakes practice, or something else entirely.
