...
And our main cognitive limitation is that: attention. Linguists used to be very focused on trying to imagine grammars that use as little memory as possible on the assumption that our memory for individual words and expressions was quite limited. The thinking was \enquote{why remember \textit{jump}, \textit{jumped}, \textit{jumps}, \textit{jumping} along with all the other regular verb forms when you can just remember \textit{jump} and then produce the rest on the fly with rules?} It seems more likely that memory retrieval may sometimes be a problem, but \enquote{storage capacity} is practically unlimited \citep{Erman2000, JackendoffAudring2018}.
