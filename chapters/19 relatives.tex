...
\begin{tcolorbox}[title=Check Your Understanding: Gap Analysis, colback=yellow!10!white, colframe=orange!75!black]
...
\ea \label{ex:relatives-visualized}
    ...
    \ex[]{He set them a \tikzmarknode{d}{\textcolor{xGreen}{problem}}, \ob\tikzmarknode{G}{\emph{the an\tikzmarknode{f}wer to \textcolor{xGreen}{which}}} they found \tikzmarknode{H}{\gap} \cb~online.}
    \z
\z
...
\Needspace{22pt}
\ea
    \ea[]{The places \ob \emph{to which} we go \gap \cb~are nearby.}\hfill[\textsc{articulated}]}
    \ex[*]{The places \ob \op that\cp~\emph{to~~~} we go \gap \cb~are nearby.}\hfill[\textsc{basic}]}
    \z
\z
\ea
    \ea[]{The friend \ob \emph{whose bike} we borrowed \gap \cb~is nearby.}\hfill[\textsc{articulated}]}
...
\ea
    \ea[]{That \ob which we like \gap \cb~is nearby.}\hfill[\textsc{articulated}]}
...
\ea
    \ea[]{We saw a car \ob which we recognized \gap but which we didn't expect \cb.}\hfill[\textsc{articulated}]}
...
\ea
    \ea[]{A jerk, \ob which I am not \gap \cb, would have left.}\hfill[\textsc{articulated}]}
...
\begin{tcolorbox}[title=Pattern Recognition: Restrictive vs Non-restrictive, colback=yellow!10!white, colframe=orange!75!black]
...
\begin{tcolorbox}[title=Quick Self-Check: Relative Construction Types, colback=yellow!10!white, colframe=orange!75!black]
...
This chapter analyzed relative constructions as complex anaphoric structures involving gaps, antecedents, and systematic structural variation:

\begin{itemize}[noitemsep]
    \item \textbf{Anaphoric gap relationship}: Relative clauses have gaps where pronouns would appear in independent clauses, creating dependency between antecedent and missing element
    \item \textbf{Basic vs focused distinction}: Basic relatives use \textit{that}-marking or no marking, while focused relatives use fronted relative words (\textit{who}, \textit{which}, \textit{where}, etc.)
    \item \textbf{Syntactic restrictions}: Relative word selection depends on gap function -- \textit{where} requires locative gaps, \textit{who} requires human subjects/objects, etc.
    \item \textbf{Gap position variability}: Gaps can occupy subject, object, complement, or adjunct positions within the relative clause structure
    \item \textbf{Processing complexity}: Subject gaps easier than object gaps, proximity effects between antecedent and gap affect comprehension difficulty
    \item \textbf{Integrated vs supplementary function}: Modifier relatives (restrictive) integrate into NP structure while supplement relatives (non-restrictive) attach to clause level
    \item \textbf{Punctuation and intonation}: Supplementary relatives require comma separation and distinct intonation contours in speech
    \item \textbf{Antecedent type expansion}: Supplementary relatives allow VP, clause, AdjP, PP, and AdvP antecedents beyond typical NP antecedents
    \item \textbf{Infinitival relative constraints}: Require prepositional relative phrases, no overt subjects, formal register restriction
    \item \textbf{Fused relative structure}: Relative word simultaneously functions as antecedent and gap binder (\textit{what/whoever/whichever} forms)
    \item \textbf{Recursive embedding potential}: Relative clauses can contain other relative clauses, creating theoretically unlimited structural complexity
    \item \textbf{Resumptive pronoun variation}: Non-standard retention of pronouns in gap positions, more acceptable in complex embeddings
\end{itemize}
