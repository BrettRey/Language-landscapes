...
\begin{tcolorbox}[title=Diagnostic techniques in this chapter, colback=gray!10!white, colframe=gray!75!black]
This chapter introduces analytical methods for understanding relative constructions:
\begin{itemize}[noitemsep]
    \item \textbf{Gap Analysis} -- Identifying missing elements and their syntactic functions within relative clauses
    \item \textbf{Antecedent-Gap Mapping} -- Tracing anaphoric relationships between head nouns and gap positions
    \item \textbf{Relative Word Selection Tests} -- Understanding why certain relative words appear with specific gap types
    \item \textbf{Integration vs Supplementation} -- Distinguishing restrictive from non-restrictive interpretations
    \item \textbf{Clause Type Classification} -- Systematically categorizing basic vs focused relative structures
\end{itemize}
These diagnostic tools reveal how relative constructions create complex modification relationships through syntactic gaps and anaphoric dependencies.
\end{tcolorbox}
...
\begin{itemize}[noitemsep]
    \item \textbf{Anaphoric gap relationship}: Relative clauses contain gaps where pronouns would appear in independent clauses, creating dependency between antecedent and missing element
    ...
    \item \textbf{Gap position variability}: Missing elements can occupy subject, object, complement, or adjunct positions within the relative clause structure
...
Understanding relative constructions requires systematic analysis of gap-antecedent relationships, structural classification, and processing factors rather than relying on traditional prescriptive rules about \textit{that} vs \textit{which} usage.
