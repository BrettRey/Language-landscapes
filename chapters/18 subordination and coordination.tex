...
\begin{tcolorbox}[title=Analytical techniques in this chapter, colback=gray!10!white, colframe=gray!75!black]
This chapter introduces key methods for analyzing complex sentence structure:
\begin{itemize}[noitemsep]
    \item \textbf{Hierarchical Analysis} -- Identifying levels of embedding and dependency relationships between clauses
    \item \textbf{Substitution Tests} -- Using pro-forms and movement to test structural relationships
    \item \textbf{Coordination Tests} -- Applying conjunction reduction and parallelism diagnostics
    \item \textbf{Matrix-Subordinate Mapping} -- Tracking how clauses can be simultaneously main/matrix or subordinate/matrix
    \item \textbf{Structural Ambiguity Resolution} -- Using syntactic tests to disambiguate complex coordination patterns
\end{itemize}
These analytical tools reveal how subordination and coordination create the architectural framework for complex meaning expression in English.
\end{tcolorbox}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Matrix-subordinate relationships}: Clauses can simultaneously be main/matrix (containing subordinates) or subordinate/matrix (embedded but containing other subordinates)
    ...
    \item \textbf{Content clause types}: Declarative (\textit{that}-clauses), basic interrogative (\textit{whether/if}), focused interrogative (\textit{wh}-words), and mandative (subjunctive/modal)
