...
Prepositions are some of the most common words in English. Behind only \mention{the} and \mention{and}, \mention{of} occurs roughly once every 50 words. Other extremely common prepositions are \mention{in}, \mention{to}, \mention{for}, \mention{with}, \mention{on}, \mention{at}, \mention{from}, and \mention{by}, all of which are among the 30 most frequently used words in English.%TODO: add corpus citation for "of" frequency and top-30 claim (COCA? BNC?)
...
\ea At which I ceas't, and listen'd \emph{them} a while. (1634)\label{ex:listen}%TODO: cite OED s.v. listen for this and next example
\z
...
The basic meaning of \mention{up} is `away from the ground', as in \textit{The balloon went up.}
...
\begin{itemize}
    \item   \textit{Even \emph{if} I spend 10 hours, I can't fix it.} $\rightarrow$ \textit{Even I spend 10 hours, I can't fix it.}\footnote{Although this conditional  \mention{if} is a preposition, there is also a subordinator \mention{if} used in certain interrogative clauses such as \textit{I wonder if she lives here.} See Chapter \ref{ch:subord-coord}.}
...
The examples in (\ref{ex:chairoff}) are superficially similar to those in (\ref{ex:pickup}, \ref{ex:pickit}, \& \ref{ex:takeoff}), but \mention{off} functions as the head of the Mod:PP \textit{off your chair}, in this case, and not as a particle. That's why the preposition and the NP can't be reversed, as shown in (\ref{ex:chairoffb}).
...
(\ref{ex:frontinga}) is how you would ask this if you hadn't heard clearly, were asking a clarifying question, or were expressing surprise: the full \textit{about what} PP appears in the VP after the head verb. In (\ref{ex:frontingb}), which is the normal way to ask the question, the NP \mention{what} has been \textsc{fronted} and the preposition \mention{about} is \textsc{stranded}. In (\ref{ex:frontingc}), which is an awkwardly formal way of asking the question, the whole PP is fronted.
