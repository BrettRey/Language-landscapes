...
\begin{itemize}[noitemsep]
    \item Distinguish between adverbs and adverb phrases (AdvPs) and their characteristics
...
\begin{itemize}[noitemsep]
    \item \textbf{Test 1: Modification Patterns} -- PPs accept modifiers like \textit{right}, \textit{just}, \textit{straight}; AdvPs accept \textit{very}, \textit{too}, \textit{so}
    \item \textbf{Test 2: Predicative Complement Function} -- PPs can follow \textit{be}, \textit{seem}, \textit{remain}; AdvPs typically cannot
    \item \textbf{Test 3: Coordination Patterns} -- PPs coordinate with PPs; AdvPs coordinate with AdvPs; mixed coordination is ungrammatical
    \item \textbf{Test 4: Modification within NPs} -- PPs can modify nouns; AdvPs generally cannot (with rare exceptions)
...
\begin{itemize}
   ...
   \item \textbf{Modifiable by \textit{very}:} Also like adjectives, adverbs can often be modified\is{modification, modifier|(} by \textit{very}: \textit{very well}, \textit{very quickly}, \textit{very often}, \textit{very hard}, \textit{very long}, \textit{very soon}, etc.
\end{itemize}
...
It's quite unusual for AdvPs to function as complements\is{complement, complementation|)}, but they do occur. \textit{She }[\textit{treated it \uline{carefully}}]\textit{.} \textit{He }[\textit{worded his response \uline{very precisely}}]\textit{.} \textit{Wait }[\textit{until \uline{later}}]\textit{.} \textit{{\ob}Before \uline{long}{\cb}, they were ready}
...
\begin{tcolorbox}[title=Try This: Progressive Practice, colback=green!10!white, colframe=green!75!black]
\textbf{Step 1 (Easy):} Try modifying typical \textit{-ly} adverbs with \textit{right}: \textit{*right quickly}, \textit{*right carefully}. Notice how these sound wrong.

\textbf{Step 2 (Medium):} Now try modifying words like \textit{before}, \textit{here}, \textit{down} with \textit{right}: \textit{right before}, \textit{right here}, \textit{right down}. These sound natural.
...
\textbf{Frequency vs. Function:} Can you explain why highly frequent words like prepositions often develop grammatical rather than just lexical meanings? Think about the examples with \textit{by}, \textit{of}, and \textit{to} above.
\end{tcolorbox}
...
\noindent Prepositions like those in (\ref{ex:compoundprep}) have internal structure. For example \textit{apart} is, historically \textit{a--} + \textit{part}. Sometimes, single prepositions are written with a space, like those in (\ref{ex:complexprep}). Despite the space, there's good reason (which I won't go into here; see \cite[622]{Huddleston2002}) to think of these as single words. Finally, the three prepositions in (\ref{ex:compareprep}) are a little like adjectives\is{adjective, adjective phrase (AdjP)} and adverbs in that they have \textit{--er} and \textit{--est} forms. Again, there are good reasons to think they are prepositions, but for those, you should consult \textcite[609]{Huddleston2002}.
...
\begin{tcolorbox}[title=Learning Strategy, colback=blue!10!white, colframe=blue!75!black]
...
\begin{tcolorbox}[title=Apply Your Knowledge, colback=green!10!white, colframe=green!75!black]
\textbf{Test the Rule:} Before reading further, try this exercise:
\begin{itemize}[noitemsep]
    \item Find three examples where \textit{since} takes different types of complements
    \item Find two examples where \textit{before} appears with no complement
    \item How would traditional grammar classify these same words in each case?
\end{itemize}
\textit{(If you're having trouble, think about: \textit{since yesterday}, \textit{since you left}, \textit{I haven't seen her since}.)}
\end{tcolorbox}

\subsubsection*{The problem of words like \textit{before}} \label{sec:likebefore}

The word \textit{before} serves as a prime example of the challenges in categorizing prepositions. Traditional grammar\is{grammar, traditional} sometimes labels \textit{before} as an adverb in contexts like \textit{I've seen it \uline{before}} with no complement, as a preposition in \textit{before \uline{dinner}}, and as a so-called ``subordinating conjunction''\is{conjunction@``conjunction''!subordinating} in \textit{before \uline{he arrives}}, where there is a clausal complement. This tripartite classification is not only cumbersome but also conceptually problematic.
...
\begin{tcolorbox}[title=Words like \textit{before}, colback=white]
   There aren't actually that many  ``words like \textit{before}'', those allowing three types of complement: Obj:NP, clause, or none. Apart from \textit{before}, we have \textit{after} and \textit{since}. But the list of prepositions that take clausal complements is much larger. It includes: \textit{after}, \textit{although}, \textit{as}, \textit{assuming}, 
       \textit{because}\is{preposition, preposition phrase (PP)!because@\textit{because}}, \textit{before}, 
       \textit{considering}, 
...
Most English teachers subscribe to the widespread view of prepositions being words that always take Obj:NP complements, and when they're exposed to the view of prepositions I'm explaining here, they typically reject it as ``difficult''. If you already have one way of looking at things, it is clearly difficult to change your mind. There's no question about that. But for English-language learners, this is the simpler system. It dramatically reduces the number of ``exceptions'' and it's obviously easier for students to learn that \textit{after} is always a preposition than it is to try and understand when it's an adverb, when it's a preposition, and when it's a ``subordinating conjunction''. The choice then becomes: a system easier for teachers, or a system easier for students?\footnote{It's also hardly an innovation, being at least a quarter of a millennium old \citep{Hunter1784}.}\is{pedagogy}\is{preposition, preposition phrase (PP)!flexible-complement view|)}\is{complement, complementation|)}\is{preposition, preposition phrase (PP)|)}

\begin{tcolorbox}[title=Pause and Reflect, colback=yellow!10!white, colframe=orange!75!black]
\textbf{Pedagogical Implications:} Consider the teaching challenge here. Which approach would be easier for your students: learning that \textit{before} has three different categories depending on context, or learning that \textit{before} is always a preposition that can take different types of complements (or none)? What are the pros and cons of each approach?
\end{tcolorbox}
...
The verb \textit{listen} is another that has seen changes in its complementation patterns\is{verb, verb phrase (VP)!complementation of|(} over time. In its earlier usage, \textit{listen} could take either a direct object (\ref{ex:listen}) or
...
\textbf{Think About Your Language Background:} How does your first language handle spatial and temporal relationships? Do you use the same words/particles for both, or different ones? Understanding these differences can help explain why preposition learning is challenging—the conceptual mappings vary across languages.
\end{tcolorbox}
...
Anyone learning English would struggle to find systematicity in this hodgepodge. And the list goes \emph{on} and \emph{on}.\is{preposition, preposition phrase (PP)!selection of (collocation)|)}\is{collocation!with PP|)}

\begin{tcolorbox}[title=Teaching Tip, colback=green!10!white, colframe=green!75!black]
\textbf{Managing Student Expectations:} When students ask "Why do we say 'on medication' but 'at a distance'?" it's important to acknowledge that many preposition choices are simply conventional. Rather than seeking logical explanations for every usage, focus on helping students notice patterns and develop strategies for remembering common collocations.
\end{tcolorbox}
...
But English \textit{in} has senses that are not shared with Italian. For instance, larger locations like cities are usually \textit{in} in English but \textit{a} (`at') in Italian. English also uses \textit{in} for smaller vehicles like cars, where Italian would use \textit{su} (`on'), as in buses or boats for English. Conversely, Italian prefers \textit{in} for large vehicles that would normally attract \textit{on} in English.\il{Italian}\is{preposition, preposition phrase (PP)!challenges for learners|)} These are issues of metaphor selection.
...
Notice that you can only fall off your chair because you're on it, but you're not on your jacket, so \textit{off your jacket} is not a phrase in (\ref{ex:takeoff}i).\is{particle|)}\is{modification, modifier|)}\is{phrasal verbs|)}

\begin{tcolorbox}[title=Practice Consolidation, colback=green!10!white, colframe=green!75!black]
\textbf{Phrasal Verb Analysis:} Try analyzing these examples using the particle test:
\begin{itemize}[noitemsep]
    \item \textit{turn off the light} vs. \textit{turn off the highway}
    \item \textit{pick up the phone} vs. \textit{walk up the stairs}
    \item Can you reverse the order in each case? What does this tell you about the structure?
\end{itemize}
\textbf{Reflection:} How does understanding particle movement help you distinguish phrasal verbs from verb + PP combinations?
\end{tcolorbox}
...
The structure of these phrases can be difficult to grasp when just written out in text, as in this chapter. That's why it's often useful to show them in a graphic format that linguists call \textsc{syntax trees} or just \textsc{trees}. Such trees are the focus of the next chapter.\is{fronting|)}\is{preposition stranding|)}
...
\begin{itemize}
    ...
    \item Preposition choice and usage present significant challenges for English learners due to their often arbitrary nature and cultural determination.\is{culture!and preposition choice}
\end{itemize}
...
Still, taking a flexible-complement view\is{preposition, preposition phrase (PP)!flexible-complement view|(} of prepositions, as we naturally do with verbs, will go a long way to eliminating most of the ``exceptions'' found in the traditional analysis and simplify the treatment of modification and complementation in clauses and other phrases.\is{preposition, preposition phrase (PP)!flexible-complement view|)}

\section{Glossary}

\begin{description}
\item[adverb phrase (AdvP)] A phrase headed by an adverb that typically functions as a modifier, expressing meanings like manner, frequency, degree, or time.

\item[preposition phrase (PP)] A phrase headed by a preposition that can function as either a modifier or complement, expressing spatial, temporal, and other relational meanings.

\item[flexible-complement view] The analysis that treats prepositions as heads that can take various types of complements (NPs, clauses, PPs, or no complement), contrasting with the traditional NP-complement view.

\item[NP-complement view] The traditional analysis that defines prepositions primarily as words that must govern a noun phrase, leading to problematic reclassifications when no NP follows.

\item[phrasal verb] A multi-word verb consisting of a verb plus a preposition (particle) that together create an idiomatic meaning different from the sum of their parts (e.g., \emph{give up}, \emph{look after}).

\item[particle] A preposition that combines with a verb to form a phrasal verb and can often be separated from the verb by its object (e.g., \emph{turn off the light} / \emph{turn the light off}).

\item[stranded preposition] A preposition that appears without its complement due to movement operations, such as in questions (\emph{What are you looking at?}) or relative clauses.

\item[preposition fronting] The movement of an entire prepositional phrase to the front of a clause, typically in formal registers (\emph{At what are you looking?}).

\item[complement selection] The idiosyncratic requirement that certain verbs, adjectives, or nouns take specific prepositions as complements (e.g., \emph{depend on}, not \emph{*depend of}).

\item[spatial preposition] A preposition that primarily expresses location or direction in physical space (e.g., \emph{in}, \emph{on}, \emph{under}, \emph{through}).

\item[temporal preposition] A preposition that primarily expresses relationships in time (e.g., \emph{before}, \emph{after}, \emph{during}, \emph{since}).

\item[intransitive preposition] A preposition that appears without any complement, traditionally misclassified as an adverb in some analyses (e.g., \emph{come in}, \emph{fall down}).
\end{description}
