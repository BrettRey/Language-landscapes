...
In each italicized adverb phrase, the bold adverb is the head: \textit{\emph{clearly}}, \textit{\emph{very}}, \textit{very \emph{clearly}}, \textit{not so \emph{well}}, etc.
...
\begin{itemize}[noitemsep]
   \item in VPs: \textit{She sings \emph{beautifully}}. \textit{He ran \emph{quickly} to the store}.
   \item in AdjPs: \textit{The movie was \emph{incredibly} boring}. \textit{She is \emph{almost} ready}.
   \item in other AdvPs: \textit{He runs \emph{very} quickly}.  \textit{She sings \emph{quite} beautifully}.
   \item in clauses: \textit{\emph{Hopefully}, she will arrive soon.} \textit{\emph{Surprisingly}, he didn't enjoy the concert.}
   \item in PPs: \textit{He lives \emph{right} in the city centre}. \textit{She sat \emph{just} behind the curtain}
   \item before NPs: \textit{\emph{Even} the thought of it is frightening.} \textit{Everyone's presence was comforting, \emph{particularly} yours.}
   \item but not in NPs: *\textit{The \emph{quickly} runner won.} *\textit{She bought the \emph{beautifully} dress.}\footnote{\citet{payne2010} document rare exceptions such as \textit{the demand \emph{internationally} for lithium}.}
\end{itemize}
...
\begin{itemize}[noitemsep]
   ...
   \item \textit{\emph{two hours} before he arrived}, \textit{\emph{a kilometer} down the road}.
...
\emph{from high school}}). In its most recent usage, \mention{graduate} means `successfully
exit an educational institution upon completion of the course of study' and takes
an Obj:NP denoting the school (\textit{I graduated \emph{high school}}).
...
evenings generally (\mention{in the morning}), months and seasons (\mention{in January}, \mention{in (the) summer}), and years (\mention{in 2023}). There is little rhyme or reason to these choices.
...
\ea \label{ex:fronting}
    ...
    \ex[]{\emph{\emph{What} do you disagree }[\emph{\emph{about}} \gap]\emph{?}\hfill [fronting with stranding]}\label{ex:frontingb}
