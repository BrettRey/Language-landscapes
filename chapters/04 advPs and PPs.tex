...
\begin{itemize}[noitemsep]
   ...
   \item \textbf{Grade:} Like adjectives, adverbs are often gradable: \mention{well}, \mention{better}, \mention{best}; \mention{fast}, \mention{faster}, \mention{fastest}; \textit{more skillfully}, \textit{least helpfully}, etc.
   \item \textbf{Modifiable by \mention{very}:} Also like adjectives, adverbs can often be modified\is{modification, modifier|(} by \mention{very}: \textit{very well}, \textit{very quickly}, \textit{very often}, \textit{very hard}, \textit{very long}, \textit{very soon}, etc.
\end{itemize}
...
It's quite unusual for AdvPs to function as complements\is{complement, complementation|)}, but they do occur. \mention{She }[\textit{treated it \emph{carefully}}]\textit{.} \mention{He }[\textit{worded his response \emph{very precisely}}]\textit{.} \mention{Wait }[\textit{until \emph{later}}]\textit{.} \textit{{\ob}Before \emph{long}{\cb}, they were ready}
...
\textbf{Step 1 (Easy):} Try modifying typical \mention{-ly} adverbs with \mention{right}: \textit{*right quickly}, \textit{*right carefully}. Notice how these sound wrong.

\textbf{Step 2 (Medium):} Now try modifying words like \mention{before}, \mention{here}, \mention{down} with \mention{right}: \textit{right before}, \textit{right here}, \textit{right down}. These sound natural.
...
Preposition phrases can include modifiers, especially adverbs \mention{even}, \mention{just}, and \mention{almost}; and distinctively \mention{right}, \mention{straight}, \mention{clear}, and \mention{way}. They also include measure phrases like \textit{two metres} as modifiers, but rarely degree modifiers.
...
\begin{tblsframed}{Words like \mention{before}}
   There aren't actually that many  \enquote{words like \mention{before}}, those allowing three types of complement: Obj:NP, clause, or none. Apart from \mention{before}, we have \mention{after} and \mention{since}. But the list of prepositions that take clausal complements is much larger. It includes: \mention{after}, \mention{although}, \mention{as}, \mention{assuming}, 
       ...
       \mention{for},
       \mention{given}, 
       \mention{if}, \mention{in} + \mention{that}, \textit{in case}, \textit{in order} + \mention{that}, 
       \mention{lest}, \mention{notwithstanding} + \mention{that}, \mention{now} + \mention{that}, \textit{on condition}, \mention{once}, 
...
them, each of the following is possible: \textit{speak about}/\mention{of}/\mention{on}/\textit{to prepositions}. But
...
The basic meaning of \mention{up} is \enquote{away from the ground}, as in \textit{The balloon went up.}
This is extended to \enquote{a higher/larger amount or value}, as in \textit{The price has gone up.}
From there, we get the idea of \enquote{conscious/functioning}, presumably from the amount
of functioning, as in \textit{The system's back up.} If something is functioning, then it is
\enquote{prepared/able}, as in \textit{Are you feeling up to it.} And from being able to complete
something, we get the meaning of \enquote{completion/totality}, as in \textit{They ate it up.} 

This chain of development may not be entirely historical, but when it's laid out
that way we can see how (possibly) one might get from \enquote{away from the ground}
to \enquote{completion/totality}. But the motivation is not one that a typical learner or
...
\begin{itemize}[noitemsep]
    ...
    \item weekends in the UK: \textit{at the weekend} (with the stress on \mention{end}), but not in
    North America, where we say \textit{on the weekend} (with the stress on \mention{week})
...
evenings generally (\textit{in the morning}), months and seasons (\textit{in January}, \mention{in }(\mention{the})\textit{ summer}), and years (\textit{in 2023}). There is little rhyme or reason to these choices.
...
\small Identify \mention{all} adverbs and prepositions; label them \textsc{adv} / \textsc{prep}.
...
\begin{enumerate}[nosep, leftmargin=*]
  ...
  \item Explain the source of the error *\textit{She arrived for the station late}.
\end{enumerate}
\end{tblsframedsymbol}
...
\textbf{Part A}
\begin{enumerate}[nosep, leftmargin=*]
    \item \mention{frankly} \textsc{adv}; \mention{to} \textsc{prep}
    \item \mention{right} \textsc{adv}; \mention{before} \textsc{prep}
    \item \mention{abroad} \textsc{prep}; \mention{for} \textsc{prep}
    \item \mention{on} \textsc{prep}; \mention{just} \textsc{adv}; \mention{inside} \textsc{prep}
    \item \mention{in} \textsc{prep}; \mention{out} \textsc{prep}
\end{enumerate}
...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item It should be \textit{She arrived at the station late}. The PP functions as a complement in the VP. The verb \mention{arrive} selects a PP headed by \mention{at}, not by \mention{for}.
\end{enumerate}
