...
\begin{itemize}[noitemsep, leftmargin=*]
    ...
    \item Analyse how prepositions can take various types of complements (NPs, clauses, PPs, or none)
...
\begin{itemize}[noitemsep, leftmargin=*]
    \item Test 1: Modification patterns. PPs accept modifiers like \mention{right}, \mention{just}, \mention{straight}; AdvPs accept \mention{very}, \mention{too}, \mention{so}
    \item Test 2: Predicative complement function. PPs can follow \mention{be}, \mention{seem}, \mention{remain}; AdvPs typically can't
    \item Test 3: Coordination patterns. PPs coordinate with PPs; AdvPs coordinate with AdvPs; mixed coordination is sometimes possible but often ungrammatical
    \item Test 4: Modification within NPs. PPs can modify nouns; AdvPs generally can't (with rare exceptions)
    \item Particle movement test: For phrasal verbs, particles can move around NP objects but not pronoun objects
\end{itemize}
These tests provide systematic evidence for the flexible-complement view of prepositions.
\end{tblsfilled}

\section{Adverbs and their phrases}
...
\begin{itemize}[noitemsep]
   \item manner: \mention{easily}, \mention{quickly}, \mention{silently}
   \item time and frequency: \mention{already}, \mention{always}, \mention{often}, \mention{rarely}, \mention{temporarily}
   \item degree: \mention{almost}, \mention{thoroughly}, \mention{very}
   \item order: \mention{initially}, \mention{last}, \mention{next}
   \item certainty: \mention{definitely}, \mention{likely}, \mention{probably}
   \item domain: \mention{politically}, \mention{scientifically}, \mention{technically}
   \item evaluation: \mention{fortunately}, \mention{happily}, \mention{well}
   \item speech act: \mention{confidentially}, \mention{frankly}, \mention{honestly}
   \item connective: \mention{furthermore}, \mention{however}, \mention{thus}.
   \item etc.
\end{itemize}
...
\begin{itemize}[noitemsep]
   \item adjective + \mention{-ly}: The following are quite common examples: \mention{exactly}, \mention{recently}, \mention{usually}, \mention{quickly}, \mention{clearly}, \mention{particularly}, \mention{completely}.
   \item grade: Like adjectives, adverbs are often gradable: \mention{well}, \mention{better}, \mention{best}; \mention{fast}, \mention{faster}, \mention{fastest}; \mention{more skillfully}, \mention{least helpfully}, etc.
   \item modifiable by \mention{very}: Also like adjectives, adverbs can often be modified\is{modification, modifier|(} by \mention{very}: \mention{very well}, \mention{very quickly}, \mention{very often}, \mention{very hard}, \mention{very long}, \mention{very soon}, etc.
\end{itemize}

\subsubsection*{Adverb phrases}
\label{sec:advps}
...
The italicized adverbs are the heads of the following adverb phrases: \textit{\textbf{clearly}}, \textit{\textbf{very}}, \textit{very \textbf{clearly}}, \textit{not so \textbf{well}}, etc.
...
\begin{itemize}[noitemsep]
   \item in VPs: \textit{She sings \textbf{beautifully}}. \textit{He ran \textbf{quickly} to the store}.
   \item in AdjPs: \textit{The movie was \textbf{incredibly} boring}. \textit{She is \textbf{almost} ready}.
   \item in other AdvPs: \textit{He runs \textbf{very} quickly}.  \textit{She sings \textbf{quite} beautifully}.
   \item in clauses: \textit{\textbf{Hopefully}, she will arrive soon.} \textit{\textbf{Surprisingly}, he didn't enjoy the concert.}
   \item in PPs: \textit{He lives \textbf{right} in the city centre}. \textit{She sat \textbf{just} behind the curtain}
   \item before NPs: \textit{\textbf{Even} the thought of it is frightening.} \textit{Everyone's presence was comforting, \textbf{particularly} yours.}
   \item but not in NPs: *\textit{The \textbf{quickly} runner won.} *\textit{She bought the \textbf{beautifully} dress.}\footnote{\citet{payne2010} document rare exceptions such as \textit{the demand \textbf{internationally} for lithium}.}
\end{itemize}

\subsubsection*{Adverb phrases and complements}
\label{sec:AdvPs+Comps}
It's quite unusual for AdvPs to function as complements\is{complement, complementation|)}, but they do occur. \mention{She }[\textit{treated it \textbf{carefully}}]\mention{.} \mention{He }[\textit{worded his response \textbf{very precisely}}]\mention{.} \mention{Wait }[\textit{until \textbf{later}}]\mention{.} \textit{{\ob}Before \textbf{long}{\cb}, they were ready}

Similarly, AdvPs rarely allow complements of any kind, but there are some adverbs that do: [\textit{\textbf{happily} for them}]\textit{ it had worked}; [\textit{\textbf{independently} of his parents}]; [\textit{\textbf{appropriately} for the situation}]. In some cases, the complements are indirect: \textit{\textbf{more} important \textbf{than that}}, \textit{\textbf{as} good \textbf{as you}}, \textit{\textbf{so} bad \textbf{that it's scary}}, \textit{\textbf{too} late \textbf{to change}}.


\subsection{Words that are not actually adverbs}
\is{adverb, adverb phrase (AdvP)|)}
...
Step 1 (easy): Try modifying typical \mention{-ly} adverbs with \mention{right}: \mention{*right quickly}, \mention{*right carefully}. Notice how these sound wrong.

Step 2 (medium): Now try modifying words like \mention{before}, \mention{here}, \mention{down} with \mention{right}: \mention{right before}, \mention{right here}, \mention{right down}. These sound natural.

Step 3 (challenging): What does this difference suggest about the categories these words belong to?
\end{tblsframedsymbol}

\section{Prepositions and preposition phrases}
...
Part of the reason for this is that along with their basic meanings, they tend to have grammatical functions. For example, \mention{by} indicates the agent in passive constructions, like \textit{The book was written \textbf{by her}}. Similarly, \mention{of} can denote possession, origin, or indeed any relationship, as in \textit{The sound \textbf{of the waves}.} If we can say \mention{I gave her the note} or \textit{I gave the note \textbf{to her}}, how much meaning can \mention{to} really be adding?
...
\begin{itemize}[noitemsep]
   ...
   \item But they also take a variety of other complements, including PP (\textit{out \textbf{of the box}}), Clause (\textit{before \textbf{I arrived}}), and AdjP (\textit{for \textbf{free}}), or no complement at all.
\end{itemize}
...
\begin{itemize}[noitemsep]
   \item spatial: \mention{in}, \mention{on}, \mention{under}, \mention{between}
   \item temporal: \mention{before}, \mention{after}, \mention{during}, \mention{until}
   \item directional: \mention{to}, \mention{from}, \mention{towards}, \mention{away}
   \item causal: \mention{because}, \mention{due}, \mention{since},
   \item etc.
\end{itemize}

\noindent Often prepositions can denote different types of relations (e.g., \mention{before 6:00}; \mention{before the mirror}).

\subsubsection*{Functioning as the head of a preposition phrase}

Every preposition phrase has a preposition at its head:

\begin{itemize}[noitemsep]
   \item \textit{\textbf{In} the park}, \textit{\textbf{during} the movie}, \textit{\textbf{under} the table}
\end{itemize}

\subsubsection*{Preposition phrases as modifiers and complements}

Preposition phrases function as modifiers in NPs and VPs:

\begin{itemize}[noitemsep]
   \item Modifying NPs: \textit{the book \textbf{on the shelf}}, \textit{the man \textbf{with the hat}}
   \item Modifying VPs: \textit{slept \textbf{beside a tree}}, \textit{studies \textbf{during the day}}
\end{itemize}

They can also function as complements:

\begin{itemize}[noitemsep]
   \item In VPs: \textit{She is \textbf{in the garden}}. \textit{He talked \textbf{to us}}. \textit{I received it \textbf{from them}}.
   \item In NPs: \textit{knowledge \textbf{of finance}}, \textit{interest \textbf{in movies}}, \textit{desire \textbf{for love}}.
   \item In AdjPs: \textit{eager \textbf{for work}}, \textit{happy \textbf{about the change}}, \textit{proud \textbf{of you}}.
   \item In PPs: \textit{down \textbf{from the cottage}}, \textit{due \textbf{to the time}}, \textit{except \textbf{for that}}.
\end{itemize}
...
Preposition phrases can include modifiers, especially adverbs \mention{even}, \mention{just}, and \mention{almost}; and distinctively \mention{right}, \mention{straight}, \mention{clear}, and \mention{way}. They also include measure phrases like \mention{two meters} as modifiers, but rarely degree modifiers.

\begin{itemize}[noitemsep]
   \item \textit{\textbf{almost} to the door}, \textit{\textbf{even} without it},  \textit{\textbf{just} before noon}.
   \item \textit{\textbf{right} in front of the house}, \textit{\textbf{straight} up the tree}, \textit{\textbf{clear} to the horizon}, \textit{\textbf{way} out west}.
   \item \textit{\textbf{two hours} before he arrived}, \textit{\textbf{a kilometer} down the road}.
   \item *\textit{\textbf{very} in the room}, *\textit{\textbf{so} down the well}, *\textit{\textbf{too} on the table}.
\end{itemize}

\subsubsection*{Complements of prepositions}
\label{sec:PP-complementation}

While Obj:NP is the most common complement in PPs, they can take various complements.

\begin{itemize}[noitemsep]
   \item Obj:NP: \textit{on \textbf{the table}}, \textit{with \textbf{John}}
   \item Comp:VP \textit{before \textbf{going to bed}}
   \item Comp:PP: \textit{out \textbf{of the box}}, \textit{from \textbf{behind the clock}}
   \item Comp:Clause: \textit{before \textbf{I arrive}}, \textit{of \textbf{whether it works or not}}
   \item Comp:AdjP: \textit{for \textbf{free}}, \textit{as \textbf{possible}}
   \item (no complement): \mention{above}, \mention{home}, \mention{here}, \mention{now}\, \mention{then}, \mention{where}
\end{itemize}

\subsection{Distinguishing preposition phrases from adverb phrases: Key syntactic differences}
\label{sec:distinguishing-pps-advps}
\is{tests, diagnostic|(}
...
\subsubsection*{Test 1: Modification patterns}
\label{sec:test-modification}
...
Building complexity gradually: Start by mastering the modification test (Test 1) before moving to the other tests. Each test provides a different kind of evidence, but the modification test is often the most accessible starting point for distinguishing AdvPs from PPs.
\end{tblsfilledsymbol}

\subsubsection*{Test 2: Function as predicative complement}
\label{sec:test-predcomp}
...
\subsubsection*{Test 3: Coordination patterns}
\label{sec:test-coordination}
...
\subsubsection*{Test 4: Modification within noun phrases}
\label{sec:test-np-mod}
...
\subsection{Preposition phrases and their complements}
...
Test the rule: Before reading further, try this exercise:
\begin{itemize}[noitemsep, leftmargin=*]
...
The word \mention{before} serves as a prime example of the challenges in categorizing prepositions. Traditional grammar\is{grammar, traditional} sometimes labels \mention{before} as an adverb in contexts like \textit{I've seen it \textbf{before}} with no complement, as a preposition in \textit{before \textbf{dinner}}, and as a so-called ``subordinating conjunction''\is{subordinator} in \textit{before \textbf{he arrives}}, where there is a clausal complement. This tripartite classification is not only cumbersome but also conceptually problematic.
...
If we accept that prepositions can take various complements, including clauses, then there's no need to posit a long list of \enquote{subordinating conjunctions}. We also keep the category of adverbs from becoming too unruly. Recall from Section~\ref{sec:advps} that AdvPs rarely allow or function as complements. What then are we to make of \textit{before \textbf{now}} if both \mention{before} and \mention{now} are adverbs?
...
student(s) (\textit{The university graduated \textbf{a large class} this year}). Later, \mention{graduate} came
...
\textbf{from high school}}). In its most recent usage, \mention{graduate} means `successfully
exit an educational institution upon completion of the course of study' and takes
an Obj:NP denoting the school (\textit{I graduated \textbf{high school}}).

For earlier speakers of English, the newer patterns would likely have sounded ungrammatical;
...
And a \mention{to} PP is only the most common complement. The following either were previously possible (marked with \textsuperscript{\dag}) or still are.

\begin{itemize}[noitemsep]
    \item \textsuperscript{\dag}\mention{listen of}: `hear tell of'
    \item \textsuperscript{\dag}\mention{listen on}: equivalent to `listen to'
    \item \mention{listen {\op}out{\cp} for}, \mention{listen after}: `be eager or make an effort to catch the sound of', `endeavor to hear or hear of'
...
\ea \label{ex:to-meanings}
    \ea location: In this picture, Diana is standing to my left.
    \ex contact: Apply the soap directly to the stain for best results.
    \ex attachment: He added a fence to the garden.
    \ex event: The captain went to the boaters' rescue.
    \ex comparison: The design of this sweater is inferior to that one.
    \z
\z

\begin{tblsfilledsymbol}{Cross-linguistic awareness}{bulbon}
Think about another language: How does a non-English language you know handle spatial and temporal relationships? Does it use the same words/particles for both, or different ones? Understanding these differences can help explain why preposition learning is challenging: The conceptual mappings vary across languages.
\end{tblsfilledsymbol}
...
Consider \mention{at} meaning `something or someone's exact point'. We use this \mention{at} in the following contexts:\is{idiomaticity}

\ea\label{ex:at-temporal-uses}
    \ea clock times: \mention{at 12:01} but not \mention{at January 8}
    \ex weekends in the UK: \mention{at the weekend} (with the stress on \mention{end}), but not in North America, where we say \mention{on the weekend} (with the stress on \mention{week})
    \ex nights, usually without a determiner: \mention{at night}, but not *\mention{at morning}
    \ex meals, also without a determiner: \mention{at dinner}, but not *\mention{at rice}
    \ex ages: \mention{at the age of 27}, but not *\mention{at 2023}
    \z
\z
While the patterns in (\ref{ex:at-temporal-uses}) appear idiomatic in modern English (with preposition selection often depending on syntactic factors like determiner presence rather than clear conceptual mapping) their historical development likely reflects cognitive\is{culture!and preposition choice|)} metaphors of time as space.\is{metaphor|)}\is{preposition, preposition phrase (PP)!selection of (collocation)|(} The original metaphorical extension of spatial \mention{at} to temporal domains suggests early English speakers conceptualized certain temporal references (clock times, nights, meals) as points. This cognitive foundation helps explain why these particular temporal expressions crystallized with \mention{at}, even though modern speakers may no longer feel this metaphorical mapping.
...
\begin{itemize}[noitemsep]
    \item   \textit{Even \textbf{if} I spend 10 hours, I can't fix it.} $\rightarrow$ \mention{Even I spend 10 hours, I can't fix it.}\footnote{Although this conditional  \mention{if} is a preposition, there is also a subordinator \mention{if} used in certain interrogative clauses such as \mention{I wonder if she lives here.} See Chapter~\ref{ch:subord-coord}.}
    \item \textit{That depends \textbf{on} your ability.} $\rightarrow$ \mention{That depends your ability.}
\end{itemize}

\subsection{Phrasal verbs}\label{ssec:phrasal-verbs}\is{phrasal verb|(}\is{idiomaticity|(}
...
To be clear, they're often not \term{phrases} in our technical sense. Still,
...
\begin{itemize}[noitemsep]
    ...
    object is a pronoun: \textit{take \textbf{them} off} vs \mention{take off your shoes}.
...
Notice that you can only fall off your chair because you're on it, but you're not on your jacket, so \mention{off your jacket} is not a phrase in (\ref{ex:takeoff}i).\is{particle|)}\is{modification, modifier|)}\is{phrasal verb|)}

\begin{tblsframedsymbol}{Practice consolidation}{pencil}
Try analysing these examples using the particle test:
\begin{itemize}[noitemsep, leftmargin=*]
...
\ea \label{ex:fronting}
    ...
    \ex[]{\emph{\textbf{What} do you disagree }[\emph{\textbf{about}} \gap]\emph{?}\hfill [fronting with stranding]}\label{ex:frontingb}
...
Part A: recognition\\
Identify \mention{all} adverbs and prepositions; label them \term{adv} / \term{prep}.
...
Part B: diagnostics\\
For each word, apply the named test and decide \term{adv} or \term{prep}.
\begin{enumerate}[nosep, leftmargin=*]
