...
\begin{tcolorbox}[title=Learning Objectives, colback=blue!5!white, colframe=blue!75!black]
...
\begin{tcolorbox}[title=Tests used in this chapter, colback=gray!10!white, colframe=gray!75!black]
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Passive alternation test}~-- Check if objects can become subjects in passive constructions: \textit{They reported a theft} → \textit{A theft was reported}
    \item \textbf{Pronoun case test}~-- Verify subjects use nominative case (\textit{I}, \textit{he}, \textit{she}) while objects use accusative (\textit{me}, \textit{him}, \textit{her})
    \item \textbf{Determiner ordering test}~-- Check if elements can appear in \_\_\_ \textit{of the} NOUN pattern (\textit{some of the people}, \textit{many of the changes})
...
\ea
    ...
    \ex[]{In the garden, \emph{the flowers} \textbf{bloom} brightly.}
    \ex[]{Quickly, \emph{I} \textbf{need} to go to the store.}
    \ex[]{\emph{Water} \textbf{is} boiling on the stove.}
    \ex[]{\textbf{Did} \emph{you} meet him at noon?}
    \ex[]{Without delay, \emph{the meeting} \textbf{was} adjourned.}
    \ex[]{\emph{The book} \textbf{lies} forgotten on the shelf.}
    \ex[]{Quietly, \emph{the night} \textbf{falls}.}
    \ex[]{Guys, \textbf{can} \emph{you} help me?}
    \ex[]{If \emph{it} \textbf{works}, \emph{I}'\textbf{ll} be pleased.}
    \ex[]{\emph{She} \textbf{likes} it when \emph{everything} \textbf{works} out.}
    \z
\z
...
\ea
    ...
    \ex[]{\emph{That the stars are visible} surprises me.\hfill[Clause]}
    ...
    \ex[]{In the library, \emph{reading quietly} is expected.\hfill[Participial VP]}
    \ex[]{\emph{What you know} can help.\hfill[Relative NP]}
    \ex[]{\emph{Who you see} isn't our concern.\hfill[Relative NP]}
    \ex[]{\emph{Eager} is good.\hfill[AdjP]\is{adjective, adjective phrase (AdjP)!as subject}}
...
\ea
    ...
    \ex Subjects denoting patients/recipients:
        ...
        \ex[]{\emph{They} got a package.}
        \z
    \ex Subjects denoting instruments and other roles:
        \ea[]{\emph{The computer} helped me speed up the analysis.}
        \ex[]{\emph{Grammar} is part of language.}
        \ex[]{\emph{Studying grammar} seems useful.}
        \ex[]{\emph{These things} can hurt.}
        \ex[]{\emph{Who you're friends with} isn't our concern.}
        \ex[]{Here, \emph{to believe wholeheartedly} is not always a virtue.}
        \ex[]{\emph{That the stars are visible} surprises me.}
        \ex[]{\emph{They}'re at school.}
        \ex[]{\emph{The broom} fell over.}
        \ex[]{\emph{It} always surprises me that I can see the stars here.}
        \z
    \z
...
In a \textit{to} infinitival VP, the subject appears in the accusative case after \textit{for}. \textit{It's not for \emph{me} to say.} \textit{It's time for \emph{us} to go.} \textit{I've been waiting for \emph{them} to ask.} \textit{Is there a way for \emph{him} to try again.}
...
Unfortunately, it's not a perfect test. Other elements may control agreement\is{agreement!complications and exceptions}. This can be some dependent in the subject, as in \textit{\emph{A number of }\textbf{people} have arrived.} Or with dummy \textit{there}\is{dummy pronoun!and subject--verb agreement}, it's the complement of the head verb: \textit{\emph{There} was \emph{something} on the floor.} \textit{\emph{There} were \emph{some things} on the floor.}\is{agreement!subject--verb|)}\is{subject (Subj)!subject--verb agreement|)}
...
\ea  \label{ex:first-person-drop}
    ...
    \ex[]{Love this song.}
    \ex[]{Went to the store earlier.}
    \ex[]{Oh! See what you mean.}
...
\ea  \label{ex:second-person-drop}
    \ea[]{Feeling OK?}
    \ex[]{Think this looks good?}
    \ex[]{Ever been to Paris?}
    \ex[]{Know what I mean?}
    \ex[]{Want some coffee?}
    \ex[]{Need a hand with that?}
    \z
\z
...
\begin{tcolorbox}[title=Check Your Understanding: Subject Properties, colback=yellow!10!white, colframe=orange!75!black]
Test your grasp of subject complexity:
\begin{itemize}[noitemsep]
    \item Can you explain why \textit{There were some books} has \textit{some books} controlling agreement, not \textit{there}?
    \item In what contexts is subject dropping acceptable in English? When is it problematic?
    \item How would you help a student who says \textit{*Is very difficult} instead of \textit{It's very difficult}?
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Determiners: A brief history\is{determiner (Det)!history of the term}, breakable, colback=white]
...
\begin{itemize}[noitemsep]
   \item AdjPs typically function as modifiers in NPs\is{adjective, adjective phrase (AdjP)!as modifier}: \textit{the \emph{happy} child}
   \item AdvPs can modify in any type of phrase except NPs\is{adverb, adverb phrase (AdvP)!as modifier}: \textit{\emph{very} quickly}
   \item PPs can function as modifiers in most types of phrases: \textit{the book \emph{on the shelf}}
   \item Relative clauses function mostly as modifiers in NPs: \textit{the book \emph{that I bought}}
   \item Even NPs can sometimes function as modifiers\is{noun, noun phrase (NP)!functions of NP!modifier}: \textit{It snowed \emph{last week}, I met her \emph{this morning}}, an \emph{old folks'} home, the \emph{faculty} office.
   \item And DPs\is{determinative, determinative phrase (DP)!as modifier}: \textit{It wasn't \emph{all that} difficult, We drank \emph{almost all} the water.}
\end{itemize}
...
\begin{tcolorbox}[title=Pattern Recognition: Modifiers vs Complements, colback=yellow!10!white, colframe=orange!75!black]
Notice the systematic differences that help distinguish modifiers from complements:
\begin{itemize}[noitemsep]
    \item \textbf{Optionality}: Modifiers can usually be removed (\textit{the tall student} → \textit{the student}), while complements often can't (\textit{proud of her} → *\textit{proud})
    \item \textbf{Selection}: Complements are selected by specific heads (\textit{afraid OF}, not *\textit{afraid TO}), while modifiers are more flexible
    \item \textbf{Stacking}: Multiple modifiers can pile up (\textit{the tall young energetic teacher}), but complements are typically limited
\end{itemize}
\textit{Why might this distinction matter for teaching grammar effectively?}
\end{tcolorbox}\is{modification, modifier!integrated vs supplementary|)}\is{modification, modifier|)}
...
\begin{tcolorbox}[title=Quick Self-Check: Constructions, colback=yellow!10!white, colframe=orange!75!black]
Test your understanding of English constructions:
\begin{itemize}[noitemsep]
    \item What's the difference between \textit{Take a walk} (light verb) and \textit{Walk} (simple verb)?
    \item Why does \textit{Touch that and you'll regret it} mean `if you touch that'?
    \item Can you identify the result phrase in \textit{She hammered the metal flat}?
\end{itemize}
\end{tcolorbox}\is{construction!spray/load@\textit{spray}/\textit{load} alternation|)}\is{construction|)}

\section{Conclusion}

Throughout this chapter, we've examined syntactic categories and functions and the constructions that combine them. The basic relationships between categories and functions~-- like NPs serving as subjects or AdjPs as modifiers~-- form the foundation of English syntax. But as we've seen, English also provides conventionalized constructions that package these elements in more complex ways.
...
\begin{itemize}
\item Subjects are typically the first major constituent in a sentence and control verb agreement, but can be identified more reliably through grammatical tests than position alone
\item Determiners~--  usually DPs~-- appear only in NPs, typically marking definiteness and providing information about number.
\item Direct objects are almost always NPs that complete the meaning of transitive verbs and can be identified through passivization and cleft construction tests
\item Indirect objects precede direct objects and can usually be paraphrased with \emph{to} or \emph{for} phrases, representing recipients or beneficiaries of actions
\item Subject and object complements complete predicate meanings and refer back to subjects or objects respectively, often following copular or linking verbs
\item Modifiers provide additional descriptive information about head words and can typically be deleted without affecting grammatical acceptability
\item Complements are selected by the head of the phrase and often required for the phrase to be grammatical 
\item The modifier-complement distinction helps explain why some constituents are optional while others are obligatory in sentence structure
\item Constructions are form-meaning pairings that exist at various levels of abstraction, from specific phrases to general syntactic patterns
\item Understanding categories, functions, and constructions provides teachers with analytical tools for explaining grammatical patterns and helping students navigate complex sentence structures
\end{itemize}
