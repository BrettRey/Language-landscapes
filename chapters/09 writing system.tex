...
\begin{tcolorbox}[title=Practical Application: Adult Phonics, colback=blue!10!white, colframe=blue!75!black]
...
\begin{tcolorbox}[title=Reflection: Teaching in the Digital Age, colback=blue!10!white, colframe=blue!75!black]
...
\begin{tcolorbox}[title=Linguistic Insight: Historical Patterns, colback=yellow!10!white, colframe=yellow!75!black]
...
\begin{tcolorbox}[title=Diagnostic Awareness, colback=yellow!10!white, colframe=yellow!75!black]
...
Once they're up and reading in English, then, the challenge is achieving fluency, in both decoding and language comprehension. Fluency is the topic of Chapter \ref{ch:fluency}.

\section{Summary}

This chapter explored the complex relationship between English spelling and pronunciation, providing essential background for understanding reading instruction challenges:

\begin{itemize}[noitemsep]
    \item \textbf{Historical development}: English writing evolved from runes to Roman alphabet, with major changes during the Norman Conquest (French influence) and the Great Vowel Shift (pronunciation changes after spelling was fixed)
    \item \textbf{Spelling-sound mismatches}: Modern English spelling reflects historical pronunciations, creating systematic but complex grapheme-phoneme correspondences that challenge learners
    \item \textbf{Phonics limitations}: While phonics significantly helps with decoding, English's irregularities mean that whole-word recognition and contextual strategies are also essential for fluent reading
    \item \textbf{Simple View of Reading}: Reading comprehension results from the interaction of decoding skills (converting print to sound) and language comprehension (understanding meaning)
    \item \textbf{Diagnostic framework}: Teachers can assess reading difficulties by separating decoding problems from comprehension problems, leading to more targeted instruction
    \item \textbf{Adult learner challenges}: Adults may struggle with decoding due to different L1 writing systems, undiagnosed reading disabilities, or gaps in phonological awareness
    \item \textbf{Dyslexia awareness}: Reading disabilities stem from phonological processing difficulties, not visual problems, requiring explicit and systematic decoding instruction
    \item \textbf{Balanced instruction}: Effective reading teaching addresses both decoding automaticity and meaningful comprehension, avoiding both ``barking at print" and neglecting foundational skills
    \item \textbf{Reading vs. listening}: While similar to listening, reading offers learner control but lacks some context
    \item \textbf{Fluency as the goal}: Automatic decoding and comprehension together enable the real reading that adult learners need for academic and professional success
\end{itemize}
