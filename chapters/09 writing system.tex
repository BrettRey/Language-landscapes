...
\epigraph{Leaving mere\\
palimpsest left\\
to scratch literate\\
out of obliterate.}{Penn Kemp, \enquote{Cancel Culture}}

\begin{tblsframedsymbol}{Learning objectives}{bulb}
\begin{itemize}[noitemsep, leftmargin=*]
    \item Distinguish between logographic, syllabic, and alphabetic writing systems and their implications for learning
    \item Explain the relationship between phonemic awareness and alphabetic literacy acquisition
    \item Understand grapheme-phoneme correspondences in English and their complexities
    \item Apply phonics principles for teaching decoding skills to adult ESL learners
    \item Trace how historical events (the Norman Conquest, Great Vowel Shift, and printing press) shaped modern English spelling
    \item Apply the Simple View of Reading to understand decoding vs comprehension difficulties
    \item Identify signs of dyslexia and reading disabilities in adult language learners
    \item Balance decoding instruction with meaningful reading comprehension activities
    \item Evaluate the changing role of spelling in the digital age and adapt teaching accordingly
\end{itemize}
...
Phonemes, on the other hand, are deeply inaccessible to our conscious awareness. No pre-literate child could tell you that \mention{cat} has three ``sounds'' or that \mention{jump} has four. In fact, it seems that there is a reciprocal causal relationship between learning to read an alphabet\is{alphabet|(} like the one English uses and \term{phonemic awareness}\is{phonemic awareness}: you can't develop it without exposure to an alphabet and you can't successfully learn to read an alphabet without phonemic awareness.
...
Simplifying somewhat, we can say that there are three major types of writing systems in the world. The first is logographic\is{writing!system|(}. Chinese is an example of a logographic writing system. The word for `outside' in Mandarin\il{Chinese!Mandarin} is \mention{wài}, and it's written ⟨外⟩. The character ⟨外⟩ doesn't represent the phonemes or the syllable. Instead, it represents a whole word. In Cantonese\il{Chinese!Cantonese}, this same character represents \mention{ngoi}, while still meaning `outside'.

If this seems a little mysterious, it may be easier to understand the concept with the Arabic numerals, which are also logographic. For instance \mention{7}, as in \mention{7:00}, doesn't encode the phonemes /sɛvən/. Nor does it encode the syllables se-ven. \mention{7} represents a whole word, as do ⟨3⟩ in \mention{\$3} or ⟨0⟩ in \mention{0 degrees}. But while it's \mention{seven} for an English speaker, it's \mention{efta} for a Greek\il{Greek} speaker and \mention{saba} in Swahili.\il{Swahili}
...
You're, of course, familiar with the English alphabet, which is based on the Roman alphabet\is{alphabet!Roman|(}. You may not realize that \mention{alphabet} does not denote the specific set of symbols you're reading right now. Russian\il{Russian} is written in the Russian Cyrillic alphabet\is{alphabet!Cyrillic} ⟨русский⟩, and Kurdish\il{Kurdish} in the Kurdish Arabic alphabet\is{alphabet!Arabic} ⟨{\persian کوردی}⟩.

What distinguishes alphabets from other writing systems is that they (primarily) represent individual phonemes. They do this better in some languages, like Finnish\il{Finnish} and Greek\il{Greek}, and worse in others like English and French\il{French}. Finnish has more or less a one-to-one relationship between letters and phonemes, making it extremely easy to learn to decode Finnish words. English, sadly, has a many-to-many relationship, even if you ignore allophones (Section~\ref{sec:allophones}). English spelling does not so much map sounds as maintain a long and complicated correspondence with them. 

Multiple graphemes\is{grapheme|(} may represent the same phoneme:

\begin{itemize}[noitemsep]
    \item /f/ can be spelled as ⟨f⟩ \mention{\emph{f}ar}, ⟨ph⟩ \mention{\emph{ph}one}, or ⟨gh⟩ \mention{enou\emph{gh}}.
    \item /k/ can be ⟨c⟩ \mention{\emph{c}at}, ⟨k⟩ \mention{\emph{k}ite}, ⟨ck⟩ \mention{ba\emph{ck}}, ⟨ch⟩ \mention{\emph{ch}orus}, or ⟨que⟩ \mention{uni\emph{que}}.
\end{itemize}
        
Conversely, a single grapheme may represent multiple phonemes:
\begin{itemize}[noitemsep]
    \item ⟨c⟩ can represent /s/ as in \mention{\emph{c}ity}, or /k/ as in \mention{\emph{c}at}.
    \item ⟨g⟩ can represent /g/ as in \mention{\emph{g}o}, or /d͡ʒ/ as in \mention{\emph{g}iant}.
\end{itemize}
        
And, of course, there are various silent letters\is{silent letters|(}.

\begin{itemize}[noitemsep]
    \item ⟨k⟩ is silent in \mention{\emph{k}night} and \mention{\emph{k}nife}, pronounced /nʌɪt/ and /nʌɪf/ respectively.
    \item ⟨w⟩ is silent in \mention{\emph{w}rite} and \mention{\emph{w}rong}, pronounced /rʌɪt/ and /rɒŋ/.
    \item ⟨b⟩ is silent in \mention{dou\emph{b}t} and \mention{su\emph{b}tle}, pronounced /daʊt/ and /sʌtəl/.
    \item ⟨h⟩ is silent in \mention{\emph{h}onest} and \mention{g\emph{h}ost}, pronounced /ɒnɪst/ and /ɡoʊst/.
    \item ⟨e⟩ is silent in \mention{cak\emph{e}} and \mention{bit\emph{e}}, making the vowels ``long'',\footnote{I'll note again that ``long'' vowels are not necessarily longer in duration than other vowels. See the discussion in Section~\ref{sec:long-vowels}.} pronounced /keɪk/ and /baɪt/.
\end{itemize}

So, the very fact that English is alphabetic\is{writing!system} makes it difficult to learn \emph{if you haven't already learned to read an alphabet}, in other words, if you're illiterate or if you are literate only in a logographic\is{writing!system|)} or syllabic\is{writing!system|)} script. But very few people these days fall into those groups. Even Chinese speakers typically learn ``pinyin'', a way to write their language with the Roman alphabet. Beyond that, though, even if you come to English with phonemic awareness, the weak correspondences between spelling and pronunciation, between graphemes and phonemes, make it much more difficult to learn to read and write than a language like Finnish\il{Finnish}, Italian\il{Italian}, or Greek\il{Greek}, where the correspondences are quite robust.
...
While writing systems provide the framework for representing language visually, teaching methods like phonics help bridge the gap between alphabetic written symbols and spoken language. \term{Phonics} is a method of teaching reading that focuses on the relationship between the sounds (phonemes) of spoken language and the letters (graphemes) used to represent those sounds in written language. It is a crucial component of early literacy instruction, as it helps learners develop the skills to decode\is{decoding|(} unfamiliar words by sounding them out.
...
Unfortunately, as we've seen, the grapheme--phoneme correspondences\is{grapheme!grapheme--phoneme correspondence|(} can be unreliable, especially with vowels. A digraph\is{digraph} like ⟨ea⟩, for instance, may be /i/ as in \mention{teach}, /iə/ as in \mention{real}, /ɛ/ as in \mention{head}, /eɪ/ as in \mention{great}. Greater consistency can be achieved here by considering the vowel along with the coda, in other words the part of the word that's important for rhyming, often called the \term{rime}\is{rime}. For example, ⟨each⟩~is consistently /it͡ʃ/ whether the word be \mention{each}, \mention{teach}, \mention{reach}, \mention{beach}, \mention{peach}, or \mention{bleach}. Another example is ⟨ate⟩, which is quite consistently /eɪt/ in stressed\is{stress} syllables. (For a list of teachable grapheme--phoneme correspondences, see Section~\ref{sec:gpcs-list}.)

It must be admitted that there are exceptions, such as \mention{water} for ⟨ate⟩~and \mention{treachery} for ⟨each⟩, but the use of rimes as a spelling unit is generally more useful than focusing on individual vowel spellings.
...
\begin{itemize}[noitemsep]
    ...
    \item Segmenting: Learners break words down into their syllables, rimes, and phonemes (e.g., \mention{attack}~=~/æ/~+~/tæk/, \mention{tack}~=~/t/~+~/æk/, \mention{ack}~=~/æ/~+~/k/).
    \item Blending: Learners practice combining syllables, rimes, and phonemes.
    \item Phoneme manipulation: Learners engage in activities that involve adding, deleting, or substituting syllables, rimes, and phonemes within words (e.g., changing \mention{cat} to \mention{hat} by replacing /k/ with /h/).
...
\begin{tblsfilledsymbol}{Practical application: Adult phonics}{bulbon}
How does adult phonics differ from children's phonics? Adults may struggle with words they can decode but don't recognize (like \mention{instead}: is it /ɪnstɛd/ or /ɪnstid/?). How does this connect to the vocabulary development discussed in Chapter 8? What teaching strategies might address this challenge?
...
Unfortunately, finding appropriate phonics materials for adult English language learners\is{phonics!for adult ESL learners} can be a challenge. Many resources are designed for young native speakers of English and may feature words and concepts that are unfamiliar or irrelevant to adult learners from different cultural backgrounds. For example, a typical children's phonics book might focus on words like \mention{frog,} \mention{log,} \mention{mop,} and \mention{cape}, words that are usually in the vocabulary of English-speaking children but may be unknown to adult learners.

Another difference is that most English-speaking children will recognize a word once decoded, or occasionally be able to guess what a word will be from context. Adults learning English will often not know the word at all. When a grapheme has multiple possible phonemic realizations, this leaves them with no way to know whether \mention{instead}, for example, should be read as /ɪnstɛd/ or /ɪnstid/. This is another reason why it's crucial to build vocabulary as quickly as possible.

\section{Why English spelling looks this way}
\is{spelling!history of|(}

English spelling can seem chaotic. But its quirks hold clues to the language's past. Take the word \mention{knight}, for example.

\subsection{Why \mention{knight}?}

How did we ever get ⟨knight⟩ for /nʌɪt/? This seems like a reasonable question, but it gets things backwards. Instead, we should ask how we got /nʌɪt/ for ⟨knight⟩. Because when \mention{knight} was first spelled ⟨knight⟩, that was a good match for the way it was pronounced.\is{pronunciation!and spelling|(}

It really did start with /kn/. That strikes most English speakers today as very odd and hard to say.\is{phonotactics} But that's not due to any physical barrier to pronouncing /kn/. We say it all the time, in words like \mention{acknowledge} /əknɒlɪdʒ/, \mention{acne} /ækni/, \mention{technician} /tɛknɪʃən/, \mention{picnic} /pɪknɪk/, \mention{banknote} /bæŋknəʊt/, etc. What makes it appear difficult to us is that Modern English has a phonotactic prohibition against syllable-initial /kn/ (see Section~\ref{sec:phonotactics}). If you look again, you'll notice that in all the words above, the /k/ is at the end of one syllable and the /n/ is at the beginning of another.

The ⟨i⟩ was pronounced /i/, as in \mention{machine} or \mention{police}. The ⟨gh⟩ was pronounced /x/ (the sound at the end of Scottish \mention{loch} or German \mention{Bach}). This sound, a voiceless velar fricative, was once common in English but has since been lost in most dialects. Words like \mention{light}, \mention{night}, and \mention{right} all contained this /x/ sound, spelled as ⟨gh⟩. And the ⟨t⟩ was pronounced /t/. So the whole word was pronounced /knixt/.
...
Interestingly, in some dialects of English, particularly Canadian English and some Northern U.S. dialects, a phenomenon called Canadian raising\is{Canadian raising|(} occurs. In this process, the /aɪ/ diphthong is raised to [ʌɪ] before voiceless consonants. This means that while \mention{ride} is pronounced /raɪd/, \mention{right} becomes /rʌɪt/. The same applies to our word \mention{knight}, which in these dialects is pronounced /nʌɪt/ rather than /naɪt/.

This raising is thought to be a recent development, occurring long after the loss of the /x/ sound. It shows how pronunciation continues to evolve even in modern times, adding another layer to the complex history of words like \mention{knight}.
...
Many words with \enquote{silent letters}\is{silent letters|)} tell a similar story. \mention{Knife}, \mention{gnat}, \mention{pneumonia}, \mention{psychology}, \mention{debt}, \mention{salmon}, and others all preserve pronunciations from long ago. Some spellings even preserve pronunciations from other languages: \mention{ballet} and \mention{coup} keep their French\il{French} spellings, while \mention{fjord} keeps its Norwegian spelling.

Understanding these historical patterns can sometimes help us guess the meanings of unfamiliar words. Words beginning with ⟨kn⟩ might relate to joints or sudden movements, like \mention{knee}, \mention{knuckle}, or \mention{knock}. Words starting with ⟨ps⟩ often relate to the mind, like \mention{psychology}, \mention{psychic}, or \mention{psychosomatic}.

But this approach has limits. English is full of exceptions, and these patterns aren't reliable. Many words don't follow expected patterns, and some that do aren't semantically related. \mention{Knit} and \mention{knife} share a spelling pattern but not a meaning. Relying too heavily on spelling for meaning can mislead. Consider \mention{phone} (from Greek\il{Greek} \mention{phōnē}, meaning voice) or \mention{island} (which gained its ⟨s⟩ through a mistaken connection to the unrelated word \mention{isle}).
...
\subsection{How English orthography evolved}
...
\subsubsection{From runes to the Roman alphabet}
...
\caption{The Anglo-Saxon futhorc (runic alphabet). Each rune is shown with its name, transliteration, and meaning. Source: Blodcyning, own work, CC BY-SA 4.0, \url{https://commons.wikimedia.org/w/index.php?curid=76035366}}
\label{fig:futhorc}
\end{figure}

Figure~\ref{fig:futhorc} shows the Anglo-Saxon futhorc. Each rune represented a phoneme and had a name, often related to its shape or cultural significance. For example, the first rune, \mention{feoh}, meant `wealth' and represented /f/.

The futhorc included runes for sounds not found in the Latin alphabet. Of particular interest is ⟨þ⟩, which represented the ``th sounds'' in Old English\il{English!Old}, as in \mention{thorn}, which was its name. It appears in the third position of the futhorc.
...
\begin{enumerate}[noitemsep]
    ...
    \item They used ⟨þ⟩ from the futhorc for the unvoiced /θ/ and introduced ⟨ð⟩ (eth) for voiced /ð/ (as in \mention{though}).

    \item They used the runic wynn ⟨ƿ⟩ for the /w/ sound, before eventually replacing it with the double-u (⟨uu⟩, which became ⟨w⟩).

    \item The reason the double-u looks like two ⟨v⟩s instead of two ⟨u⟩s is that initially the vowel was written with the sharp shape ⟨v⟩ and the consonant as the rounded ⟨u⟩. While they switched places in Middle English, the \enquote{sharp u} continued to be used for ⟨w⟩.
\end{enumerate}
...
Notice the use of ⟨þ⟩ in \mention{modgeþanc} and ⟨æ⟩ in \mention{fæder} (father) and \mention{wæs} (was). This text shows how the Anglo-Saxons had adapted the Roman alphabet to their language while retaining some elements inspired by their runic heritage.
...
\subsubsection{How the Norman Conquest and the Great Vowel Shift reshaped spelling}
\label{sec:great-vowel-shift}

The Norman Conquest\is{Norman Conquest|(} brought profound changes. French\il{French} became the language of the nobility, while English continued among the common people. When English re-emerged as a written language in the 14th century, it had absorbed a wealth of French vocabulary, and French spelling conventions.
...
\begin{itemize}[noitemsep]
    \item \mention{bite}: Middle English /biː.tə/ became Modern English\il{English!Modern} /baɪt/
    \item \mention{beet}: Middle English /beːt/ shifted to Modern English /biːt/
    \item \mention{beat}: Middle English /bɛːt/ moved to Modern English /biːt/
    \item \mention{bate}: Middle English /baː.tə/ shifted to Modern English /beɪt/
\end{itemize}

\begin{figure}
    \includegraphics[width=0.8\textwidth]{figures/Great_Vowel_Shift2c.pdf}
...
\mention{Please} and \mention{pleasant}:
\begin{itemize}[noitemsep]
  \item In Middle English, \mention{please} was pronounced /plɛːzə/ (with a long /ɛː/), and \mention{pleasant} was /plɛzənt/ (with a short /ɛ/). The ⟨pleas⟩ sounded the same, other than that the ⟨ea⟩ of \mention{please} was drawn out longer than was that of \mention{pleasant}.
  \item The long /ɛː/ in \mention{please} shifted, along with the other long vowels. It went through /eː/ to become /i/, giving us Modern English /pliz/.
  \item The short /ɛ/ in \mention{pleasant} was unaffected, being short, resulting in Modern English /plɛzənt/.
...
The \enquote{silent e} in Modern English spelling is another fossil left by the Great Vowel Shift. Back in Middle English, that ⟨e⟩ wasn't silent at all: it was pronounced as a schwa /ə/ (like the last vowel in \mention{sofa}). So ⟨name⟩ wasn't just /neɪm/, but /naːmə/.
...
But the spelling stuck around, like a vestigial tail. Now that ⟨e⟩ serves a new purpose: it's a signal that the preceding vowel is \enquote{long}. But remember, today's \enquote{long} vowels aren't really prolonged; they're just diphthongs in many cases.

\begin{itemize}[noitemsep]
    \item \mention{bite}: Middle English /biː.tə/ $\rightarrow$ Modern English /baɪt/
    \item \mention{name}: Middle English /naː.mə/ $\rightarrow$ Modern English /neɪm/
    \item \mention{rose}: Middle English /rɔː.zə/ $\rightarrow$ Modern English /roʊz/
\end{itemize}

In each case, that final ⟨e⟩ used to be its own syllable. Now it contributes mainly to the atmosphere, a reminder of a pronunciation long gone.

This \enquote{silent e} rule isn't foolproof; English loves its exceptions. But it's a handy fossil, showing us how the Great Vowel Shift left its mark not just on our pronunciation, but on our spelling too.

\begin{tblsframedsymbol}{Linguistic insight: historical patterns}{test}
Understanding historical sound changes helps explain spelling patterns to students. Can you now explain why \mention{please} and \mention{pleasant} have different vowel sounds despite similar spellings? How might this knowledge help students remember these distinctions?
\end{tblsframedsymbol}
\il{English!Middle|)}\is{Great Vowel Shift|)}

\subsubsection{The impact of printing and classical learning}
...
Caxton often chose spellings from his native Kent dialect. For example, he preferred ⟨gh⟩ in words like ⟨right⟩ over the northern ⟨rigt⟩. At the same time, a renewed interest in classical learning led printers and scholars to modify spellings to reflect (often erroneously) Latin or Greek origins:
   ...
   \begin{itemize}[noitemsep]
     \item The silent ⟨b⟩ in ⟨doubt⟩ and ⟨debt⟩ was added to echo Latin\il{Latin} \mention{dubitare} and \mention{debitum}.
     \item The ⟨s⟩ in ⟨island⟩ was added due to a mistaken connection with Latin \mention{insula}, though the word actually comes from Old English\il{English!Old} \mention{igland}.
   \end{itemize}
...
But as the idea that English could be a language of books and learning began to take hold, there was a concurrent push for standardization. In 1755, Samuel Johnson published his \mention{A Dictionary of the English Language}. It was a massive undertaking, with 42,773 entries. Johnson didn't just list words; he included quotations showing how they were used. He also made choices about spelling, helping to establish norms.

But Johnson's dictionary wasn't the final word. Across the Atlantic in the USA, Noah Webster was working on an American dictionary. Webster's \mention{An American Dictionary of the English Language} aimed to standardize American English spelling. He's the reason Americans write \mention{color} while Brits write \mention{colour}.

Webster argued for spellings that reflected pronunciation. He succeeded with words like \mention{center} (instead of \mention{centre}) and \mention{defense} (instead of \mention{defence}). But some of his more radical ideas, like spelling \mention{women} as \mention{wimmen}, didn't catch on.
...
But there's still variation. British and American English maintain their spelling differences. Spellings like \mention{realize} and \mention{realise} coexist. Some words have multiple accepted spellings even within the same variety of English. Is it \mention{judgement} or \mention{judgment}? Both are accepted.
\is{spelling!history of|)}

\section{Spelling in the digital age}
...
But the digital revolution has shaken up the spelling world. Now we have LLMs taking our spoken words and turning them into text. Our phones and computers finish our words for us and fix our typos on the fly. Web browsers and word processors underline our mistakes and offer one-click fixes.
...
\begin{tblsfilledsymbol}{Reflection: Teaching in the Digital Age}{bulbon}
How has technology changed your approach to spelling instruction? What balance do you strike between teaching spelling patterns and teaching students to effectively use digital tools? How might this balance shift as AI becomes more prevalent in writing?
\end{tblsfilledsymbol}

\section{From reading words to really reading} \label{sec:simple-view-of-reading}

It's worth acknowledging that the main point of reading is not decoding words but understanding texts. Be that as it may, once you're able to decode words well, then reading isn't all that much different from listening. In both cases, it's mainly language comprehension. This is called the \term{Simple View of Reading}\is{Simple View of Reading|(} \citep{Gough1986} and can be expressed as the formula:
\[
...
\begin{figure}
    ...
    \caption{The four categories of readers predicted by the Simple View of Reading. By Smilingpolitely, CC BY-SA 4.0, via Wikimedia Commons.}
...
\begin{tblsframedsymbol}{Diagnostic awareness}{test}
Using the Simple View of Reading, how would you distinguish between: (1) a beginner struggling with both decoding and comprehension, (2) an intermediate student with good oral skills but poor decoding, and (3) a student with possible dyslexia? What teaching adjustments might each situation require?
\end{tblsframedsymbol}
...
One way that reading is easier is that, when you're reading, you're in charge. You can slow down, speed up, or go back. Imagine an adult learner comes across the word \mention{copacetic}. They can pause, sound out /kəʊpəsɛtɪk/, maybe even look it up. That kind of flexibility isn't available when listening to the radio, for example.

But reading presents its own challenges too. Written language often uses more complex words and longer sentences than spoken language. A business email might say, \mention{Please find attached the quarterly financial projections for your perusal,} a kind of language you'd rarely hear in the break room.
...
The Simple View from earlier in this chapter gives you the diagnostic tool. When a student struggles with reading but handles spoken English well, the problem is probably decoding, and the grapheme-phoneme patterns covered here tell you where to look. When a student decodes fluently but doesn't understand what they've read, the problem is comprehension: most often vocabulary (Chapter~\ref{ch:vocabulary}), sometimes world knowledge, and occasionally grammar. This diagnostic distinction matters because the interventions are completely different. Once decoding and comprehension are both in place, the remaining challenge is fluency, the topic of Chapter~\ref{ch:fluency}.

\section{Summary}

English spelling records where the language has been more faithfully than where it is now. That history won't make \mention{knight} easier to spell, but it will keep you from telling students that English spelling is \enquote{random}, and it gives you the Simple View of Reading as a diagnostic frame. When a student struggles with a text, the question is whether the problem is decoding or comprehension, and the answer determines what kind of help will actually work.

\is{alphabet!Roman|)}\is{alphabet|)}\is{runes|)}\is{runes|)}\is{Norman Conquest|)}\is{Great Vowel Shift|)}\is{Canadian raising|)}\is{pronunciation!historical changes in|)}\is{pronunciation!and spelling|)}\is{printing press|)}\is{orthography|)}\is{dictionary|)}\is{Simple View of Reading|)}\is{hyperlexia|)}\is{dyslexia|)}\is{reading!disabilities|)}\is{decoding!and dyslexia}\is{decoding!and comprehension}\is{decoding!definition of}\is{decoding!teaching of|)}\is{decoding|)}\is{reading!comprehension|)}\is{reading!teaching of}\is{reading|)}\is{phonics!for adult ESL learners|)}\is{phonics|)}\is{grapheme!grapheme--phoneme correspondence|)}\is{grapheme|)}\is{phoneme|)}\is{syllable|)}\is{writing!system|)}
