...
You're, of course, familiar with the English alphabet, which is based on the Roman alphabet\is{alphabet!Roman|(}. You may not realize that \textit{alphabet} does not denote the specific set of symbols you're reading right now. Russian\il{Russian} is written in the Russian Cyrillic alphabet\is{alphabet!Cyrillic} ⟨русский⟩, and Kurdish\il{Kurdish} in the Kurdish Arabic alphabet\is{alphabet!Arabic} ⟨{\persian کوردی}⟩.
...
\begin{itemize}[noitemsep]
    \item /f/ can be spelled as ⟨f⟩ \textit{\emph{f}ar}, ⟨ph⟩ \textit{\emph{ph}one}, or ⟨gh⟩ \textit{enou\emph{gh}}.
    \item /k/ can be ⟨c⟩ \textit{\emph{c}at}, ⟨k⟩ \textit{\emph{k}ite}, ⟨ck⟩ \textit{ba\emph{ck}}, ⟨ch⟩ \textit{\emph{ch}orus}, or ⟨que⟩ \textit{uni\emph{que}}.
\end{itemize}
        
Conversely, a single grapheme may represent multiple phonemes:
\begin{itemize}[noitemsep]
    \item ⟨c⟩ can represent /s/ as in \textit{\emph{c}ity}, or /k/ as in \textit{\emph{c}at}.
    \item ⟨g⟩ can represent /g/ as in \textit{\emph{g}o}, or /d͡ʒ/ as in \textit{\emph{g}iant}.
\end{itemize}
        
And, of course, there are various silent letters\is{silent letters|(}.

\begin{itemize}[noitemsep]
    \item ⟨k⟩ is silent in \textit{\emph{k}night} and \textit{\emph{k}nife}, pronounced /nʌɪt/ and /nʌɪf/ respectively.
    \item ⟨w⟩ is silent in \textit{\emph{w}rite} and \textit{\emph{w}rong}, pronounced /rʌɪt/ and /rɒŋ/.
    \item ⟨b⟩ is silent in \textit{dou\emph{b}t} and \textit{su\emph{b}tle}, pronounced /daʊt/ and /sʌtəl/.
    \item ⟨h⟩ is silent in \textit{\emph{h}onest} and \textit{g\emph{h}ost}, pronounced /ɒnɪst/ and /ɡoʊst/.
    \item ⟨e⟩ is silent in \textit{cak\emph{e}} and \textit{bit\emph{e}}, making the vowels ``long'',\footnote{I'll note again that ``long'' vowels are not necessarily longer in duration than other vowels. See the discussion in Section \ref{sec:long-vowels}.} pronounced /keɪk/ and /baɪt/.
\end{itemize}
...
How does adult phonics differ from children's phonics? Adults may struggle with words they can decode but don't recognize (like \textit{instead} - is it /ɪnstɛd/ or /ɪnstid/?). How does this connect to the vocabulary development discussed in Chapter 8? What teaching strategies might address this challenge?
...
English spelling can seem chaotic. But its quirks hold clues to the language's past. Take the word \textit{knight}, for example.

\subsection{Why \textit{knight}?}

How did we ever get ⟨knight⟩ for /nʌɪt/? This seems like a reasonable question, but it gets things backwards. Instead we should ask, how we got /nʌɪt/ for ⟨knight⟩. Because, when \textit{knight} was first spelled ⟨knight⟩, that was a good match for the way it was pronounced.\is{pronunciation!and spelling|(}
...
The \enquote{silent e} in Modern English spelling is another fossil left by the Great Vowel Shift. Back in Middle English, that ⟨e⟩ wasn't silent at all~-- it was pronounced as a schwa /ə/ (like the last vowel in \textit{sofa}). So ⟨name⟩ wasn't just /neɪm/, but /naːmə/.
...
Understanding historical sound changes helps explain spelling patterns to students. Can you now explain why \textit{please} and \textit{pleasant} have different vowel sounds despite similar spellings? How might this knowledge help students remember these distinctions?
\end{tblsframedsymbol}
