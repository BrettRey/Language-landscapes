...
Further to the left, we have the pronoun \mention{it}. This word is labeled as a noun with the subscript \textsc{pro}, indicating it's a pronoun. Following its branch up, we find it's labeled as the head of an NP, and this NP functions as the subject (Subj) of the clause.
...
Now, let's work left to right. \mention{I} is a subject NP, so we add a branch sloping down and to the left from the Clause label. The function goes above the category, so Subj: on the first line and NP below it.
...
\begin{itemize}[noitemsep]
    ...
    \item Notice that there is a complement inside a complement. This property of being able to put one thing inside a thing of the same kind is called \textsc{recursion} (or ``nesting''). \is{recursion|(} It is the reason you can say \mention{the dog that chased the cat that ate the rat that lived in the house that Jack built}, and also the reason you probably shouldn't.
\end{itemize}
...
A clause can be ambiguous, but a tree will usually disambiguate it by showing the structure. Consider \mention{Sherlock saw the man with the binoculars} in Figure~\ref{fig:binoculars}. Out of context, this is ambiguous between a sighting via the binoculars or that of a man equipped with the binoculars.
