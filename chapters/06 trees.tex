...
Now, let's work left to right. \textit{I} is a subject NP, so we add a branch sloping down and to the left from the Clause label. The function goes above the category, so Subj: on the first line and NP below it.
...
This VP has a head:V, which terminates in \textit{had}. There's also the object NP \textit{my breakfast}, so the branch from the VP should slope leftwards.
...
Finally, we'll add the object:NP \textit{my breakfast}, with its Det:DP \textit{my} and Head:N \textit{breakfast}.
...
\begin{itemize}[noitemsep]
    ...
    \item Notice that there is a complement inside a complement. This property of being able to put one thing inside a thing of the same kind is called \textsc{recursion} (or ``nesting''). \is{recursion|(} It is the reason you can say \textit{the dog that chased the cat that ate the rat that lived in the house that Jack built}, and also the reason you probably shouldn't.
\end{itemize}
