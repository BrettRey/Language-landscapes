...
Using the solar system metaphor, analyze this sentence: \textit{The students read difficult books}. Identify: (1) What is the \enquote{sun} (head) of the whole sentence? (2) What are the \enquote{planets} (major dependents)? (3) Within \textit{difficult books}, what is the head and what is the dependent?
\end{tblsframedsymbol}
\is{head (function)|)}\is{dependent (Dep)|)}\is{complement, complementation|)}\is{modification, modifier|)}\is{object (Obj)|)}\is{subject (Subj)!|)}
...
Practice the top-down approach with this sentence: \textit{She likes chocolate}. Start with \enquote{Clause} at the top, then work left to right. Remember: (1) Subject NP on the left, (2) Head VP on the right, (3) Each phrase needs its own head, (4) Every word sits at the bottom of a branch.
\end{tblsframedsymbol}
...
\begin{enumerate}[noitemsep]
    ...
    \begin{itemize}[noitemsep]
        ...
        \end{itemize}
        \item \textbf{Analysis}: Replacing \textit{all her loyalty cards} with \mention{them} indicates that it's a constituent.
    \end{itemize}
    \is{pro-form!replacement test|)}
...
Apply the constituency tests to this sentence: \textit{The teacher explained the difficult concept clearly}. Test whether \textit{the difficult concept} is a constituent by trying: (1) Coordination: \textit{...explained \emph{the difficult concept} and \emph{the easy one}}, (2) Pro-form replacement: \textit{...explained \emph{it}}, (3) Can it be moved to the front?, (4) Can it be omitted in ellipsis?
\end{tblsframedsymbol}
\is{constituency tests|)}
...
Try this ambiguous sentence: \textit{I saw the student with the telescope}. Draw two different trees to show: (1) I used a telescope to see the student, (2) The student had a telescope. Where does the PP \textit{with the telescope} attach in each interpretation?
\end{tblsframedsymbol}
