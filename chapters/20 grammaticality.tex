...
But if you're playing American football or rugby, then different rules apply, just as they do in French and German.\is{cross-linguistic variation} And if a local children's soccer league modifies the handball rule or a local community accepts and uses double modals (\textit{you might could do that}), then the soccer players, league officials, and the \textit{might could} users aren't failing to follow rules~-- they're following different but equally systematic ones.\is{double modal} \emph{Might could} is productive and grammatical for millions of Southern US speakers and flatly ungrammatical for everyone else~-- not because one group has the right rules, but because each community maintains its own conventions through use, correction, and transmission. What counts as grammatical at any given moment is what a community has entrenched, not what's logically necessary.
...
Similarly, a Vietnamese\il{Vietnamese} student tells me that \textit{She eat rồi} (`She has eaten already') is something Vietnamese--English bilinguals would say.
...
\z
Here, the present tense serves a legitimate discourse function~-- the ``historical present" for vivid narration~-- resolving what would otherwise be a meaning clash. This interpretation is facilitated by \textit{so} = `I'm about to tell you a story'.\is{historic present|)}\is{present tense!historic|)}\is{tense!present}
...
\begin{itemize}[noitemsep]
    \item \textit{*Can the have running}~-- what makes this ungrammatical?
    \item \textit{*I have 35 years} (for age) vs \textit{I have 35 years of experience}~-- Same form, different grammaticality judgments?
    \item \textit{So I go there yesterday...}~-- How does pragmatic motivation rescue apparent ungrammaticality?
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item Why does \textit{The horse raced past the barn fell} create processing difficulty despite being grammatical?
    \item How do false positives like \textit{*More people found Bush's ads negative than they did Kerry's} reveal processing limitations?
    \item What role does dependency distance play in the development of \textit{do}-support in English questions?
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item How can the same form (\textit{I go there yesterday}) be both grammatical and ungrammatical depending on context?
    \item Why might \textit{We sheared three sheeps} be acceptable in some contexts but not others?
    \item What makes Turkish-English \textit{come-di-mi} grammatical within its speech community?
\end{itemize}
