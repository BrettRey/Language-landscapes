...
\begin{tcolorbox}[title=Check Your Understanding: Form-Meaning Analysis, colback=yellow!10!white, colframe=orange!75!black]
...
\begin{tcolorbox}[title=Pattern Recognition: Processing and Change, colback=yellow!10!white, colframe=orange!75!black]
...
\begin{tcolorbox}[title=Quick Self-Check: Grammaticality Factors, colback=yellow!10!white, colframe=orange!75!black]
...
This chapter examined grammaticality as a complex, gradient phenomenon involving community conventions, cognitive constraints, and form-meaning relationships rather than binary rule-following:

\begin{itemize}[noitemsep]
    \item \textbf{Gradient acceptability}: Grammaticality exists on a continuum from highly acceptable to completely unacceptable rather than binary correct/incorrect judgments
    \item \textbf{Form-meaning pairing foundation}: Grammatical constructions are conventional mappings between linguistic forms and semantic or pragmatic meanings within specific communities
    \item \textbf{Community convention dependence}: Different speech communities, dialects, registers, and social contexts have different grammaticality standards
    \item \textbf{Multi-dimensional meaning}: Constructions encode truth-conditional semantics, pragmatic meaning, social meaning, and discourse functions
    \item \textbf{Three ungrammaticality types}: Meaningless forms, form-meaning mismatches, and unmotivated non-standard forms (lacking pragmatic justification)
    \item \textbf{Processing limitation effects}: False negatives and false positives
    \item \textbf{Communication vs grammaticality}: Meaning can be conveyed effectively without conventional grammar, highlighting the distinction between structural correctness and communicative success
    \item \textbf{Historical change pressures}: Semantic opportunities, structural conflicts, social factors, and processing constraints drive grammatical evolution over time
    \item \textbf{Domain-general bias effects}: Universal cognitive preferences (simplicity, regularity) interact with language-specific structures to shape grammatical patterns
    \item \textbf{Dependency distance constraints}: Processing limitations favor shorter dependencies between related words, influencing structural evolution
    \item \textbf{Usage-based entrenchment}: High-frequency patterns become conventionalized through repeated use, while rare forms remain susceptible to change
    \item \textbf{Pragmatic motivation rescue}: Apparent ungrammaticality can become acceptable when serving clear communicative functions (historical present, metalinguistic plurals)
\end{itemize}