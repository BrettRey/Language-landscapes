...
This chapter introduces the NICER properties as diagnostic tests for distinguishing auxiliary verbs from lexical verbs:
\begin{itemize}[noitemsep]
    \item \textbf{N}egation -- Auxiliary verbs can be directly negated with \textit{not}; lexical verbs require \textit{do}-support
    \item \textbf{I}nversion -- Auxiliary verbs can invert with subjects in questions; lexical verbs require \textit{do}-support
    \item \textbf{C}ontraction -- Auxiliary verbs can contract with \textit{not} (e.g., \textit{isn't}, \textit{won't}); lexical verbs cannot
    \item \textbf{E}llipsis -- Auxiliary verbs can be stranded in VP ellipsis; lexical verbs cannot
    \item \textbf{R}ebuttal -- Auxiliary verbs can be used with \textit{so}/\textit{too} to contradict; lexical verbs cannot
\end{itemize}
These properties collectively demonstrate the unique syntactic behavior of auxiliary verbs and explain many of the challenges they pose for English language learners.
\end{tcolorbox}
...
\textbf{Step 1 (Easy):} Start with simple examples. Apply the negation test: can you add \textit{not} directly after these verbs? \textit{is}, \textit{can}, \textit{eat}, \textit{will}

\textbf{Step 2 (Medium):} Try the inversion test with questions: \textit{She is ready} → \textit{Is she ready?} Now try: \textit{She eats apples} → ?

\textbf{Step 3 (Challenging):} Which of these can contract with \textit{not}: \textit{will}, \textit{should}, \textit{run}, \textit{think}? What does this tell you about auxiliary vs. lexical verbs?
\end{tcolorbox}
...
\textbf{Building Complexity Gradually:} Notice how the table shows different patterns: some verbs like \textit{put} have identical forms for multiple functions, while others like \textit{go} have completely different forms. Start by mastering the patterns for high-frequency verbs like \textit{be}, \textit{have}, and \textit{do} before tackling less common irregular verbs.
\end{tcolorbox}
...
\textbf{Pedagogical Implications:} Consider the teaching challenge here. How would you explain to students why \textit{I'm leaving tomorrow} uses "present tense" to talk about future time? What are the advantages and disadvantages of teaching "two tenses" vs. "three tenses" in English?
\end{tcolorbox}
...
\textbf{Think About Your Language Background:} How does your first language express modality (possibility, necessity, obligation)? Does it use auxiliary verbs like English, or different grammatical strategies? Understanding these differences can help explain why modal auxiliaries are often challenging for English language learners to master.
\end{tcolorbox}
...
\begin{description}
\item[auxiliary verb] A special class of verbs with the NICER properties (Negation, Inversion, Contraction, Ellipsis, Rebuttal), including \emph{be}, \emph{have}, \emph{do}, and modal auxiliaries.

\item[lexical verb] A verb that serves as the main predicate of a clause and carries the primary semantic content, contrasting with auxiliary verbs that provide grammatical information.

\item[NICER properties] The five distinctive syntactic properties of auxiliary verbs: Negation (direct negation with \emph{not}), Inversion (subject-auxiliary inversion), Contraction (with \emph{not}), Ellipsis (stranding in VP ellipsis), and Rebuttal (use with \emph{so}/\emph{too}).

\item[tense] A grammatical category that relates to time but is not identical to it; English has two tenses (past and present) that can express various temporal and modal meanings.

\item[aspect] A grammatical system expressing the relevance of some element of a situation, including simple, progressive, perfect, and perfect-progressive combinations.

\item[progressive aspect] An aspectual form using \emph{be} + \emph{-ing} participle that presents a situation as ongoing or in progress at a reference time.

\item[perfect aspect] An aspectual form using \emph{have} + past participle that relates a situation to a later reference point, often expressing current relevance.

\item[modal auxiliary] A type of auxiliary verb expressing modality (possibility, necessity, obligation, etc.), including \emph{can}, \emph{could}, \emph{may}, \emph{might}, \emph{must}, \emph{shall}, \emph{should}, \emph{will}, \emph{would}.

\item[modality] The expression of meanings such as possibility, necessity, permission, obligation, and futurity, typically through modal auxiliary verbs.

\item[participle] A non-finite verb form, either present participle (\emph{-ing} form) or past participle (typically \emph{-ed} or irregular forms), used in various constructions including progressive and perfect aspects.

\item[complementation] The systematic way verbs combine with other phrases (NPs, PPs, clauses) to form complete verb phrases, with different verbs selecting different types of complements.

\item[finite verb] A verb form that is marked for tense and can serve as the main verb of a clause, contrasting with non-finite forms like participles and infinitives.
\end{description}