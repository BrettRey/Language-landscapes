...
\begin{itemize}[nosep]
    \item \textit{Be} is used to form the progressive aspect\is{progressive aspect} (\textit{I \emph{am} studying}) and passive voice (\textit{The book \emph{was} written}).
    \item \textit{Have} forms the perfect aspect\is{aspect!perfect} (\textit{I \emph{have} studied}).
    \item \textit{Do}\is{do-support@\textit{do}-support|(}\is{auxiliary do@auxiliary \textit{do}|(} is used for emphasis (\textit{I \emph{do} like it}) and in questions (\textit{\emph{Do} you like it?}) and negatives (\textit{No, I \emph{don't} like it}).
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item They require a preposition phrase with \textit{of} to introduce their complement: \textit{The building \emph{of} the bridge}, not *\textit{The building the bridge}.
\end{itemize}
...
\z\z
In (\ref{ex:lexical-be}), we can't have *\textit{Be not late} or even *\textit{ben't}. In (\ref{ex:lexical-have}), we have the verb of belonging, not the one that forms the perfect aspect. We don't ask \textit{Have you a cat?} or say \textit{I haven't a cat} (at least not in most present-day North American dialects). And if we want to ask about doing your homework, we need an auxiliary \textit{do} (underlined) to go with the lexical \textit{do}: \textit{\emph{Do} you do your homework?}
...
Often the elements that seem very important in one language are simply left out in another. English speakers learning French\il{French} often want to translate the $be+-ing$ form directly, and French has a form for that: \textit{I'm doing it} can be translated as \textit{Je suis en train de le faire.} But even though you \textit{can} translate it like that, usually you wouldn't. You'd just say \textit{je le fais.} French speakers generally just don't care about the distinction between \textit{I do it} and \textit{I'm doing it}. They don't find it relevant.
...
Another characteristic of complements is that they are selected by the head\is{head (function)}, here the head verb. \textit{Read} can select NP objects, but, generally speaking, \textit{sleep} can't. \textit{Read} doesn't select AdjP complements, but \textit{seem} does.
...
But what about (b--d)? Traditionally, we might have said that \textit{sing}/\textit{singing}/\textit{sung} are still the heads, with the auxiliary verbs just ``helping out''. But the head selects particular types of complements. Modal auxiliary verbs like \textit{can} in (b) select \textsc{bare infinitival}\is{bare infinitival} VP complements~-- ``bare'' because they've been stripped of the \textit{to} you'd find in infinitivals like the complements in \textit{she wants}/\textit{ought \emph{to sing beautifully}}. That's why we get the plain-form \textit{sing} in (b). In (c), \textit{be}~-- realized as \textit{'s}~-- selects a present participial VP as its complement, and so we find the present-participle form \textit{singing}. And in (d), \textit{have}~-- as \textit{has}~-- selects a past participial VP, which is headed here by the past-participle form \textit{sung}.
