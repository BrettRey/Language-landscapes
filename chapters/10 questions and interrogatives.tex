...
\begin{itemize}[noitemsep]
    ...
    \item Identify and construct basic interrogatives using subject-auxiliary inversion
    \item Apply do-support rules for basic interrogatives lacking auxiliary verbs
    \item Analyze focused interrogatives and their interrogative phrases (who, what, where, etc.)
    \item Recognize when interrogative phrases function as subjects vs non-subjects
    \item Use inversion rules appropriately in focused interrogatives with non-subject focus
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Plain form test} -- Use \textit{be} vs \textit{is} in do-support constructions to verify verb forms
...
\begin{tcolorbox}[title=Teaching Insight, colback=blue!10!white, colframe=blue!75!black]
Understanding why do-support is cognitively difficult helps explain common learner errors like \textit{*You like pizza?} or \textit{*What you want?} Students often avoid the complex do-support system by using declarative word order with question intonation - a strategy that works in many languages but violates Standard English expectations.
\end{tcolorbox}
...
\begin{tcolorbox}[title=Diagnostic Strategy, colback=yellow!10!white, colframe=yellow!75!black]
When students make interrogative errors, use the declarative-interrogative alternation test to diagnose the problem. If they say \textit{*Who did call you?}, convert to declarative (\textit{Someone called you}) to show that \textit{who} is the subject, so no inversion is needed. This test reveals whether the error stems from overgeneralizing inversion rules.
\end{tcolorbox}
...
This chapter provided a comprehensive framework for understanding and teaching English interrogative structures:

\begin{itemize}[noitemsep]
    \item \textbf{Three interrogative types}: Basic interrogatives (yes/no questions), focused interrogatives with subject focus (no inversion), and focused interrogatives with non-subject focus (requiring inversion)
    \item \textbf{Subject-auxiliary inversion}: The fundamental mechanism for forming most English questions, where auxiliary verbs move to prenucleus position while maintaining their grammatical relationships
    \item \textbf{Do-support system}: When no auxiliary exists in the declarative, \textit{do} provides the necessary auxiliary for inversion, with tense transferring from lexical verb to \textit{do}
    ...
    \item \textbf{Teaching sequence}: Begin with memorized classroom questions, progress through subject-focus questions (easier), then tackle non-subject focus with systematic inversion practice
\end{itemize}
