...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Scope ambiguity test} -- Examine sentences like "I didn't finish homework because I was at the park" to determine what falls within negation's scope
...
\begin{itemize}[noitemsep]
    \item In "Not many students came," what has scope over what?
    \item Why does "You can't do it" mean inability rather than "ability to not do it"?
    \item What's the scope difference in "I didn't call because I was angry" vs the reading where the reason itself is negated?
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}[noitemsep]
    ...
    \item Both can appear in unexpected environments: \textit{any} in "I think anything would work" and \textit{some} in "Before you know it, there won't be some left"
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item \textbf{Jespersen's cycle}: English negation evolved predictably through seven stages, from Old English \textit{ne} to contracted forms like \textit{don't}, potentially cycling toward future innovations like \textit{en} from \textit{anything}
    ...
    \item \textbf{Negation typology}: Four main types exist—clausal (verbal and non-verbal), sub-clausal, affixal, and pragmatic—each with distinct syntactic and semantic properties
    ...
    \item \textbf{Pedagogical implications}: Understanding negation's systematicity helps teachers recognize student errors as logical overgeneralizations rather than random mistakes, enabling more effective targeted instruction
\end{itemize}
...
\begin{description}
\item[Jespersen's cycle] A seven-stage historical process describing how negation markers evolve and change in languages, named after Otto Jespersen. English evolved from Old English \emph{ne} through stages of reinforcement to modern contracted forms like \emph{don't}.
...
\item[pragmatic negation] Negation conveyed through context, tone, or implicature rather than through explicit negative markers in the sentence structure.

\item[downward-entailing context] A linguistic environment where inferences flow from general to specific (if \emph{not all students} is true, then \emph{not some students} is also true), typically licensing negative polarity items.
