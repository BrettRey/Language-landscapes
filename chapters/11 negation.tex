...
\begin{itemize}[noitemsep]
    ...
    \item Identify and use non-affirmative items (like \textit{any}, \textit{ever}, \textit{yet}) and positive polarity items appropriately
...
\begin{itemize}[noitemsep]
    \item \textbf{Non-affirmative item test} -- Use words like \textit{at all}, \textit{ever}, or \textit{yet} to identify negative clauses: if they work, the clause is likely negative
    \item \textbf{Scope ambiguity test} -- Examine sentences like \textit{I didn't finish homework because I was at the park} to determine what falls within negation's scope
    \item \textbf{Modal negation test} -- Compare \textit{can't} vs \textit{mustn't} to understand different scope relationships with modal verbs
    ...
    \item \textbf{Positive polarity item test} -- Use words like \textit{already}, \textit{some}, \textit{a fair chance} to identify contexts that resist negation
\end{itemize}
\end{tcolorbox}
...
Today, \textit{\emph{no} man shall \emph{not} do any evil} would mean that every man will do some evil. In Old English, though, that particular phrasing was emphatically negative. Of course, the same strategy in \textit{I \emph{don't} do \emph{no} evil} is perfectly clear to \is{Standard English}Standard English speakers today, and anybody claiming those two negatives make a positive is, in fact, doing evil with words. The contemporary \textit{\emph{No} man shall do any evil, \emph{neither} in words \emph{nor} in deeds} has three negatives, each propping up the next. So-called ``\is{negation!double negation}double negatives'' are still with us in some form, even in Standard English.
...
Late in stage 2, this new \textit{not} was even obligatorily paired with \textit{ne}, as in the \il{French}French \textit{Je \emph{ne} sais \emph{pas}}, propelled by the evolving sound patterns of English, coupled with the desire for clear and unambiguous communication.
...
\begin{tcolorbox}[title=\is{auxiliary verb!negative form}Negative auxiliary verbs, breakable, colback=white]\is{contraction}
...
\begin{tcolorbox}[title=Check Your Understanding: Jespersen's Cycle, colback=yellow!10!white, colframe=orange!75!black]
Before moving on, test your grasp of language change:
\begin{itemize}[noitemsep]
    \item Can you explain why phrase-initial negators like \textit{ne} are vulnerable to disappearance?
    \item What role did \textit{do}-support play in stabilizing English negation?
    \item Do you see evidence of stage 5-6 contraction in your own speech (e.g., \textit{dunno}, reduced \textit{can't})?
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Quick Self-Check: Scope, colback=yellow!10!white, colframe=orange!75!black]
Test your understanding of scope with these examples:
\begin{itemize}[noitemsep]
    \item In \textit{Not many students came}, what has scope over what?
    \item Why does \textit{You can't do it} mean `inability' rather than `ability to not do it'?
    \item What's the scope difference in \textit{I didn't call because I was angry} vs the reading where the reason itself is negated?
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Overlap between interrogatives and negatives, breakable, colback=white]
...
\begin{tcolorbox}[title=Non-affirmative items in other languages, breakable, colback=white]
...
\is{positive polarity item (PPI)|(}\textsc{Positive polarity items} (PPIs) are the yin to the yang of non-affirmative items. While the latter flourish in negative and interrogative environments, PPIs naturally occur in positive, affirmative contexts. These items, such as \textit{some}, \textit{already}, and \textit{somewhat}, sound odd or outright ungrammatical in negative sentences. Consider (\ref{ex:PPI-environments}), where \textit{a fair chance} is intended as roughly `good odds'.
...
\begin{tcolorbox}[title=Pattern Recognition: Polarity Items, colback=yellow!10!white, colframe=orange!75!black]
Notice the systematic pattern in polarity sensitivity:
\begin{itemize}[noitemsep]
    \item \textbf{Non-affirmative items} (\textit{any}, \textit{ever}, \textit{yet}) are licensed by downward-entailing contexts (negation, questions, conditionals, few)
    \item \textbf{Positive polarity items} (\textit{some}, \textit{already}, \textit{a fair chance}) resist these same contexts
    \item Both can appear in unexpected environments: \textit{any} in \textit{I think anything would work} and \textit{some} in \textit{Before you know it, there won't be some left}
\end{itemize}
...
\begin{tcolorbox}[title=The meaning of \is{no (meaning of)@\textit{no} (meaning of)}\textit{no}, breakable, colback=white]
    ...
    \ea
        \ea[]{There is no way to do it.}\phantom{[]}`It can't be done.'
        \ex[]{There is no one way to do it.}\phantom{[]}`It can be done in many ways.'
        \z
    \z
    \ea
        \ea[]{There's no two ways about it.}
        \ex[]{There are no two ways about it.}
        \z
    \z
\end{tcolorbox}
...
The revised generalization: ``Non-affirmative items prefer downward-entailing contexts, which include but aren't limited to negation and questions.'' This more sophisticated understanding explains both the core cases and the apparent exceptions.
...
\begin{tcolorbox}[title=Appropriate polarity items, breakable, colback=white]
...
This chapter explored the complexity and systematicity of negation in English, revealing how simple surface phenomena reflect deep linguistic principles:

\begin{itemize}[noitemsep]
    \item \textbf{Jespersen's cycle}: English negation evolved through predictable stages, from Old English \textit{ne} to contracted forms like \textit{don't}, potentially cycling toward future innovations like \textit{en} from \textit{anything}
    \item \textbf{Language change logic}: Changes in negation follow systematic patterns driven by phonetic erosion, semantic bleaching, and grammaticalization, showing that apparent ``errors" often recapitulate historical processes
    \item \textbf{Scope relationships}: Negation's scope determines meaning, with modal verbs showing systematic differences (\textit{can't} = inability vs \textit{mustn't} = prohibition) 
    \item \textbf{Negation typology}: Four main types exist~-- clausal (verbal \& non-verbal), sub-clausal, affixal, and pragmatic~-- each with distinct syntactic and semantic properties
    \item \textbf{Polarity sensitivity}: Non-affirmative items (\textit{any}, \textit{ever}, \textit{yet}) are licensed by downward-entailing contexts, while positive polarity items (\textit{some}, \textit{already}) resist these environments
    \item \textbf{Diagnostic tests}: Polarity items provide reliable tests for clause polarity, helping distinguish syntactically negative from positive clauses regardless of semantic interpretation
    \item \textbf{Cross-linguistic variation}: Languages differ systematically in responding to negative questions, with some following question polarity (Japanese) and others following propositional truth (English)
    \item \textbf{Semantic asymmetries}: Negation interacts with scalar contexts differently (\textit{not ten meters} means `less than' while \textit{not third in line} means `anything but third')
    \item \textbf{Phrase structure simplicity}: Despite semantic complexity, negative constructions use the same syntactic patterns as positive ones, with GAPs and co-indexing handling fronted elements
\end{itemize}

Negation exemplifies how surface complexity in language often reflects deeper systematic principles. Teachers equipped with this understanding can approach negation instruction with confidence, recognizing both the challenges and the underlying logic that governs this essential aspect of English grammar.
