...
\is{topic!chain|(}
The first sentence (\ref{ex:parents}a) gives us \textit{both my parents}. That \mention{both} is then bisected into \textit{my mother} and \textit{my father}, and finally simplified to \mention{they} in (b \& c). Pronouns like \mention{they} create coherence by linking to and renewing previously invoked referents \citep{HallidayHasan1976,BrownYule1983}.
...
The third sentence (c) shifts to the past-tense modal \mention{would} (contracted as \mention{'d}) combined with the \mention{take} VP. This \mention{would}, again means habitual past action, similar to \textit{used to}. But in the discourse, \mention{'d} is used to elaborate the details of the story.
\is{would@\textit{would}|)}
...
\z
Here the ellipsis fails because \mention{can} can't be recovered after \mention{John}. We would need \textit{John can}.
...
There are also various more or less set preposition phrases that function as connective adjuncts. Those such as \textit{for example}, \textit{in addition}, \textit{in contrast},   \textit{on the other hand}, \textit{as a result}, and \textit{in conclusion}, are invaluable, but also often overused.
...
\begin{itemize}[noitemsep]
   \item \textbf{\mention{So}}~-- marks conclusions, summaries, or topic shifts: \textit{So what you're saying is\dots}
   \item \textbf{\mention{Well}}~-- signals various transitions, often to dispreferred responses: \textit{Well, I'm not sure about that}
   \item \textbf{\mention{Now}}~-- introduces new topics or returns focus: \textit{Now, about those reports\dots}
   \item \textbf{\mention{Look/Listen}}~-- demands attention, often preceding disagreement: \textit{Look, we need to be realistic}
   \item \textbf{\mention{Actually}}~-- corrects or contradicts expectations: \textit{Actually, that's not what happened}
\end{itemize}
...
\begin{itemize}[noitemsep]
   ...
   \item \textbf{\mention{though}}~-- qualifies preceding content: \textit{It's not a bad idea, though}
...
\begin{itemize}[noitemsep]
   \item \textbf{\mention{like}}~-- marks approximation or exemplification: \textit{It was, like, really crowded}
