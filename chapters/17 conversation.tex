...
\epigraph{Now we talk in tiny circles. She asks\\
about my day about my day about my day\\
and I tell her something new each time\\
every time so I am not\\
caught in this loop}{Margret Bollerup, \enquote{Dementia and common household objects}}

\begin{tblsframedsymbol}{Learning objectives}{bulb}
\begin{itemize}[noitemsep, leftmargin=*]
    \item Analyse the structure and organization of natural conversation beyond simple turn-taking
    \item Identify key conversational phenomena including repair, overlap, and collaborative completion
    \item Understand how speakers build and maintain common ground through conversational moves
    \item Recognize the role of embodied actions (gestures, gaze, posture) in conversation
    \item Analyse how power, identity, and social relationships are constructed through talk
    \item Understand how digital communication reshapes traditional conversational patterns
    \item Apply conversation analysis techniques to authentic spoken interactions
    \item Appreciate conversation as a complex, self-organizing social system rather than rule-following
\end{itemize}
\end{tblsframedsymbol}

\includegraphics[width=0.9\textwidth]{figures/Untitled 4.jpg}
...
\item[Nao] \annotate{And you're not actually showing how it works}{Collaborative completion (\ref{subsec:grammar-interaction})}.\\

\item[Jim] \annotate{Exactly}{Alignment (\ref{sec:common-ground})}! \\\annotate{Like}{Topic shift (\ref{sssec:topic-man})} all this stuff about \term{turn-taking} and \term{repair strategies} and~-- [\annotate{points at laptop}{Deictic gesture (\ref{sec:bodies})}]

\item[Nao] [\annotate{nodding}{Back-channel (\ref{sec:common-ground})}] \\\annotate{Mm-hmm}{Common ground (\ref{sec:common-ground})}. The old \enquote{show-don't-tell} \annotate{problem}{Metacomment (\ref{subsec:knowledge})}.\\
...
\begin{sloppypar}
\begin{dialogue}
\item[Jim] Yeah! Look what just happened: you finished my sentence~-- that's collaborative completion. And when I paused before \enquote{like}? That's~--
...
\item[Nao] Right. They replace nods and \mention{mm-hmms}. 
...
\item[Nao] We'll be insufferable, analysing everything.
...
\section{Introduction} \label{sec:intro-hidden}
\is{self-organization|(}\is{emergence|(}

Take a moment to consider your last conversation. Perhaps it was with a colleague about lesson plans, a friend about weekend plans, or a stranger about the weather. On the surface, these interactions might seem straightforward, just people exchanging information or maintaining social bonds. But have you ever noticed how a simple \enquote{how are you?} can lead to anything from a quick \enquote{fine} to a twenty-minute discussion of life's complexities? Or how the same words can mean entirely different things depending on who says them, when, and how?

Conversation is central to who we are, but like fish in water, we rarely notice its true nature. We talk about the \enquote{rules of conversation} as if they were like the rules of chess, clearly defined and deliberately followed. But real conversation is more like a flock of birds wheeling through the sky: complex patterns emerge not from a rulebook, but from countless small adjustments between individuals.
...
\end{dialogue}
\z
This brief exchange shows multiple organizational levels at work. At the micro level, we see the precise timing of turns marked by coughs, yawns, and brief responses like \mention{Oh} and \mention{Mm}. At the sequence level, we see question-answer pairs (like \mention{Why?} followed by \mention{I just don't}). At the topic level, we see how the conversation shifts from a discussion about Billy's situation at work to Steph's pregnancy through a clear topic transition (\mention{Such is life... I had a word with Steph on the phone}). And at the activity level, we see how the casual familial discussion enables Gillian and Robert to cover both workplace concerns and intimate personal matters, as well as comfortable topic transitions.
...
\end{dialogue}
\z
Textbook dialogues aim to serve real purposes: they control difficulty, stage new vocabulary, and keep things readable. But the contrast with the authentic version reveals how much is lost in the smoothing: the subtle timing adjustments marked by coughs and yawns, the collaborative construction of meaning through incomplete utterances, and the organic flow of topic transitions. In the authentic conversation, Gillian's topic shift to Steph emerges naturally through \mention{Such is life...}, while in the textbook dialogue, the transition is explicitly flagged with \mention{Let's change the subject}. Similarly, the textbook version smooths out the hesitations, repairs, and incremental additions that characterize authentic speech, replacing them with complete, well-formed sentences, adding explanatory context that real participants don't need, like labeling the subject change or explicitly stating relationships that are already clear to the speakers.

The question for teachers isn't whether to use simplified dialogues (of course you will), but whether students also get regular exposure to the messier, more authentic patterns they'll encounter outside the classroom. \citet{wilson2005} found that natural conversation creates waves of coordinated timing, from split-second adjustments to longer cycles of engagement and disengagement, and these rhythms are largely absent from scripted materials.\is{rhythm (interaction)}\is{coordination (timing)}
\is{topic!management|)}

\begin{tblsframedsymbol}{Exercise: mapping temporal layers}{pencil}
Listen to any 3-minute conversation (podcast, overheard chat, phone call). Without transcribing, attempt to identify:
\begin{enumerate}[noitemsep, leftmargin=*]
    \item One moment where micro-timing affects larger structure (e.g., a brief pause that enables a topic shift)
    \item One sequence that nests inside another (e.g., clarification sequence within a story)
    \item One place where speakers orient to the activity type (e.g., \enquote{before we wrap up...})
\end{enumerate}
...
\ea
\begin{dialogue}
\item[Donna Summer] \emph{So I always felt like I had this sort of~-- people to answer to and my children, and my child at the time, was one of them. And I felt that in the future I didn't want her to say, \enquote{Mom, well, you did it.} You know? But, you know\dots}
\item[Gross] \emph{Did she ever say that?}
\item[Summer] \emph{Yeah, she did. Yes, she did, much\dots}
\item[Gross] \emph{What was your comeback?}
\item[Summer] \dots\emph{to my chagrin.\\I just told her it was a different time, and, you know, I came from a totally different life than her.}
\end{dialogue}
\z
This interaction shows several cultural patterns at work. Gross, from a New York Jewish background where \citet{tannen1984} documents a \enquote{high-involvement style}, uses quick follow-up questions that overlap with Summer's speech. Rather than appearing rude, these overlaps signal engagement and help maintain conversational momentum. Summer, in turn, uses tag questions like \mention{you know?} to check for understanding and manage turn-taking. The overall effect is of a coordinated dance between interviewer and interviewee, each adapting to the other's conversational style.
\is{turn-taking!overlap}\is{tag question}\is{question!tag}

These differences extend beyond timing patterns. Consider how Korean\il{Korean} speakers often ask about the age of their interlocutor to decide whether to use honorific verb forms that encode complex social relationships, or how Japanese\il{Japanese} conversation involves frequent use of back-channel responses like \mention{hai} and \mention{ee} (/eː/, a back-channel \enquote{yes}) that signal attention and understanding \citep{maynard1986}. Even within English-speaking communities, we find significant geographic, cultural, and situational variation in conversational practices and expectations.
\is{back-channeling}

The implications for language teaching go beyond simple awareness of difference. When students from one conversational culture enter another, they face what \citet{gumperz1982} calls \enquote{crosstalk}, systematic mismatches in conversational expectations that can lead to serious misunderstandings. A student whose first language treats overlapping speech as normal might be seen as \enquote{pushy} or \enquote{rude} by speakers who expect sequential turn-taking. Conversely, a student from a culture that values longer silences between turns might appear \enquote{unengaged} or \enquote{slow to respond} in certain contexts.
\is{crosstalk}

While it would be unrealistic to attempt to learn what conversational style to adopt in each situation, it can be very useful to be aware that such differences exist. This awareness can help students navigate unfamiliar interactional patterns with more confidence and less anxiety. Rather than trying to master every local convention, they can develop a more flexible approach, understanding that when conversation feels awkward or uncomfortable, the issue may be cultural differences in conversational style rather than personal failings or language deficiency. For teachers, this means spending less time prescribing \enquote{correct} conversational behaviour and more time helping students recognize and reflect on different patterns of interaction they encounter.
\is{culture!and conversation|)}

\begin{tblsframedsymbol}{Exercise: cultural patterns in action}{pencil}
\begin{enumerate}[noitemsep, leftmargin=*]
    \item Listen to any publicly available interview or conversation between people from different backgrounds. Can you identify moments where conversational styles might differ? What evidence do you see of participants adjusting to each other?
    \item In group conversations you participate in, notice: Who speaks when? Are there consistent patterns in how turns are distributed? Do these patterns change based on topic or setting?
    \item Consider the last time a conversation felt \enquote{awkward} or \enquote{off.} Using concepts from this section, what might explain the discomfort beyond individual personalities?
\end{enumerate}
\end{tblsframedsymbol}
...
\ea
  \ea
  \begin{dialogue}
      \item[Rich] \emph{if you bring it intuh them}
      \item[Carol] \emph{ih don't cost yuh nothing}
  \end{dialogue}
  \ex
  \begin{dialogue}
      \item[David] \emph{so if one person said he couldn't invest}
      \item[Kerry] \emph{then I'd have ta wait}
  \end{dialogue}
  \ex
  \begin{dialogue}
      \item[Dan] \emph{when the group reconvenes in two weeks}
      \item[Roger] \emph{they're gunna issue strait jackets}
  \end{dialogue}
  \z
\z
In each case, speakers collaborate to build a complex sentence across turns. The first speaker produces a dependent clause (\mention{if...} or \mention{when...}) that projects a particular grammatical shape for what must follow. The second speaker then completes the structure, often starting with words like \mention{then} that mark the completion of the compound structure. As \citet{lerner1991} demonstrates, these \enquote{compound turn-constructional units} aren't accidents; they're systematic practices that speakers use to coordinate their contributions.

The grammar here isn't just organizing words into sentences; it's providing resources for real-time collaboration. Certain grammatical structures, like \mention{if-then} constructions, create natural pivot points where another speaker can jump in to complete the emerging utterance. We find similar patterns with relative clauses (\mention{the guy who...}), reported speech (\mention{she told me that...}), and list structures (\mention{first...second...}). These patterns show how deeply grammar is woven into the fabric of real-time interaction.

We find similar patterns with other grammatical structures, as in this example from \citet{szczepek2000}.

\ea 
\begin{dialogue}
   \item[CO] \emph{and people \textsc{also};} \\
   \emph{who've never been \textsc{close} friends of hers;} \\
   \emph{but who'd \textsc{bend over backwards};} \\
   \emph{for this \term{woman.}}
   \item[AL] \emph{but are \textsc{tired of bending over backwards.}}
   \item[CO] but they \textsc{still do.} \\
   \emph{we \textsc{all still do.}}
\end{dialogue}
\z
This exchange shows how grammar supports the collaborative development of a complex idea. After CO completes what could be a finished turn, AL continues the thought. Though AL's addition lacks its own subject, it fits smoothly into the established pattern. The grammar here provides a framework for speakers to build meaning together: CO sets up a structure that AL can extend, and then CO acknowledges and elaborates on this extension, moving from \mention{they} to the more inclusive \mention{we}.
...
\ea
\begin{dialogue}
\item[A] \emph{Pass me that thing}
\item[B] \emph{Which--}
\item[A] \emph{The blue one. On the shelf. Next to the lamp.}
\end{dialogue}
\z
Here the grammar of reference unfolds dynamically as the speakers work to establish shared understanding. Rather than producing a complete noun phrase like \mention{the blue thing on the shelf next to the lamp}, speaker A builds the reference piece by piece, responding to B's evident uncertainty.
...
\ea
\begin{dialogue}
\item[Me] \emph{What do you mean? What did people used to think about dinosaurs?}
\item[Student] \emph{They think they crawl like} [demonstrates with hand movement]
\item[Me] \emph{They thought they walked like this?} [imitates movement]
\item[Student] \emph{Yes, like, like, uh gecko. But now they think more like bird.}
\item[Me] \emph{Right, I see, more like birds. Standing upright.}
\end{dialogue}
\z
...
\ea
    \ea
    \begin{dialogue}
    \item[A] \emph{You coming tonight?}
    \item[B1] \emph{Can't.}
    \item[A] \emph{Oh, okay.}
    \end{dialogue}
    \ex
    ...
    \item[A] \emph{You coming tonight?}
    \item[B2] \emph{Can't.} \\
       \emph{Got this thing with my sister.} \\
       \emph{She needs a ride to the airport.}
    \end{dialogue}
    \z
\z
B1's single-word response forms a complete TCU. It answers the question and A treats it as adequate. B2's response contains three TCUs, each potentially complete on its own, but built together to give a fuller explanation. The speaker may have sensed that \mention{can't} alone would seem too abrupt, or that \mention{got this thing} needed elaboration.
...
\ea
\begin{dialogue}
\item[Teacher] \emph{What's the capital of France?}
\item[Student] \emph{Um... Pa...}
\item[Teacher] \emph{Par...}
\item[Student] \emph{Paris!}
\end{dialogue}
\z
The student's \mention{um} and incomplete \mention{Par} aren't sentences, but they function as meaningful TCUs here. They show the student is working on the answer, and the teacher treats them as legitimate contributions by offering support.
...
\ea
\begin{dialogue}
\item[Parent] \emph{What's that?} [points]
\item[Child] \emph{A buh buh.}
\item[Parent] \emph{Yes, a bird! What's the bird doing?}
\item[Child] \emph{Eee!}
\item[Parent] \emph{Eating, that's right. The bird is eating some seeds.}
\end{dialogue}
\z
...
\ea
\begin{dialogue}
\item[A] \emph{Did you see that email from Sandra?}
\item[B] \emph{About the meeting?}
\item[A] \emph{No, the budget thing.}
\item[B] \emph{Oh right, yeah. The cuts.}
\item[A] \emph{Pretty bad, huh?}
\item[B] \emph{Twenty percent across the board.}
\item[A] \emph{And that's just the start.}
\end{dialogue}
\z
Like the parent and child, these speakers work together to establish what they're talking about. They check understanding (\mention{About the meeting?}), repair misunderstandings when they arise (\mention{No, the budget thing}), and build shared interpretations (\mention{Pretty bad, huh?}). This process of establishing and maintaining shared understanding (what psychologists call \term{common ground}) is central to all conversation.

When you're talking with a friend, you probably don't think consciously about how you show you're following along. You nod, make little sounds like \mention{mm-hmm}, maybe repeat key words, or ask brief questions. These aren't just social niceties; they're part of what \citet{clark1991} call \enquote{grounding}, the moment-by-moment work of establishing mutual understanding.

\begin{tblsframedsymbol}{Observation exercise: grounding in action}{test}
Pay attention to your next conversation and notice:
\begin{itemize}[noitemsep, leftmargin=*]
    \item How do you show you're following along? (nods, \emph{mm-hmm}, \emph{right}, \emph{okay})
    \item What happens when someone doesn't give these signals?
    \item How do speakers check that they're being understood?
    \item What repair strategies emerge when understanding breaks down?
\end{itemize}
This awareness of grounding work reveals the collaborative effort required for even simple conversations.
\end{tblsframedsymbol}
...
\ea
\begin{dialogue}
\item[A] \emph{So you go down Bank Street}
\item[B] \emph{mm-hmm}
\item[A] \emph{past the library}
\item[B] \emph{right}
\item[A] \emph{and there's this big red building}
\item[B] \emph{the old post office?}
\item[A] \emph{yeah, exactly, and just after that}
\item[B] \emph{mm-hmm}
\item[A] \emph{there's a little side street}
\end{dialogue}
\z
B isn't just being polite here. Each response shows not only that B is listening but that specific pieces of information have been understood. \mention{mm-hmm} acknowledges receipt, \mention{right} claims understanding, and \mention{the old post office?} demonstrates understanding by offering relevant additional information.

When understanding breaks down, speakers have various ways to repair the problem. Sometimes they catch their own troubles:

\ea
\begin{dialogue}
\item[Teacher] \emph{Turn to page fifty-- sorry, fifteen.}
\item[Student] \emph{Fifteen?}
\item[Teacher] \emph{Yes, fifteen.}
\end{dialogue}
\z
...
\ea
\begin{dialogue}
\item[A] \emph{We need to defenestrate the problem.}
\item[B] \emph{Defen--?}
\item[A] \emph{Oh, sorry~-- throw it out. Defenestrate means throw out a window.}
\item[B] \emph{Ah, okay. Bit formal!}
\item[A] \emph{Yeah, showing off really} [laughs]
\end{dialogue}
\z
...
\begin{tblsframedsymbol}{Exercise: tracking common ground}{pencil}
In your next extended conversation, notice:
\begin{enumerate}[noitemsep, leftmargin=*]
    \item How often do speakers explicitly check understanding versus assuming it?
    \item What triggers repair: mishearing, misunderstanding, or something else?
    \item Can you identify a moment where common ground is assumed incorrectly? How is this discovered and resolved?
\end{enumerate}
...
The term \enquote{body language} is often used for these phenomena, but it's misleading. It suggests that bodily actions are just a kind of add-on code that translates mental states into visible signals: crossed arms mean defensiveness, leaning forward means interest, and so on. But actual embodied interaction isn't really like that. Consider this imagined interaction at a construction site:

\ea
\begin{dialogue}
\item[Worker 1] \emph{So we need to }[points to beam]\emph{ move all these and then--}
\item[Worker 2] \emph{Yeah, all along here }[traces line with finger]\emph{ up to the--}
\item[Worker 1] \emph{Right, but first we gotta }[mimes lifting motion]\emph{ all the old ones}
\item[Worker 2] \emph{Oh yeah, those'll have to come out}
\end{dialogue}
\z

\begin{tblsframedsymbol}{Exercise: grammar as real-time resource}{pencil}
\begin{enumerate}[noitemsep, leftmargin=*]
    \item Find three examples in your daily conversations where someone completes another person's grammatical structure. What types of constructions seem to invite collaborative completion?
    \item Notice how speakers build reference incrementally (like the \enquote{Pass me that thing} example). Document one instance where reference unfolds piece by piece rather than being fully specified initially.
\end{enumerate}
\end{tblsframedsymbol}
...
\begin{itemize}[noitemsep]
  ...
  \item Metaphoric gestures represent abstract concepts; watch people's hands as they discuss ideas \enquote{building on each other} or time \enquote{flowing forward} \is{gesture!metaphoric}
  \item Beat gestures mark rhythm and emphasis, often synchronized with the prosodic structure of speech \is{gesture!beat}
\end{itemize}
The timing of these gestures is remarkably precise. When someone explains a spiral staircase by saying \enquote{it goes up like [hand spirals upward] to the second floor}, the spiraling gesture begins exactly with \enquote{up} and continues through \enquote{like}. This precise timing reflects deep connections between speech and gesture systems. Even more intriguingly, listeners process speech and gesture as an integrated package; in experimental studies, people often can't remember whether they got a particular piece of information from words or gestures \citep{gurney2013}.
...
\begin{itemize}[noitemsep]
  ...
  \item Quick greetings: Standing, bodies at 45 degrees, partly oriented toward exits, maintaining what \citet{kendon1990} calls \enquote{temporary with} arrangements that signal the interaction's brief nature
...
\begin{tblsframedsymbol}{Exercise: bodies and space}{pencil}
\begin{enumerate}[noitemsep, leftmargin=*]
    \item Without recording, observe a conversation in a public space. Map the spatial arrangement and note any changes. What triggers repositioning?
    \item In your next face-to-face conversation, consciously suppress all gestures for 30 seconds. What happens to your speech? To the interaction?
    \item Watch any video conversation with sound off for 1 minute. List what information you can extract purely from bodily conduct. Then watch with sound. What crucial information required the verbal channel?
\end{enumerate}
\end{tblsframedsymbol}
\is{gesture|)}\is{embodiment|)}

These arrangements aren't static; people work constantly to maintain and adjust them as interaction proceeds. Watch a group of friends walking and talking: they'll continuously adjust their pace and positioning to maintain what Kendon calls an \enquote{F-formation}, an arrangement that lets everyone see each other's faces and gestures easily.
...
\ea
\begin{dialogue}
\item[A] \emph{We're having a barbecue on Saturday. You should come!}
\item[B] \emph{Oh, that sounds great! What time?}
\item[A] \emph{Around three?}
\item[B] \emph{Perfect. Can I bring anything?}
\end{dialogue}
\z
A's first turn doesn't just convey information about a barbecue; it extends an invitation (see Section~\ref{sec:speech-acts}). This creates what conversation analysts call a \enquote{conditional relevance}: B now needs to respond in a way that addresses the invitation. B could accept (as here), decline, or hedge, but ignoring the invitation entirely would be noticeably absent and potentially face-threatening.

But sequences can be much more complex. Consider this example from a doctor's office:

\ea
\begin{dialogue}
\item[Doctor] \emph{How can I help you today?}
\item[Patient] \emph{Well, I've been having these headaches...}
\item[Doctor] \emph{Mm-hmm}
\item[Patient] \emph{And they're different from my usual ones?}
\item[Doctor] \emph{Different how?}
\item[Patient] \emph{They're like, behind my eye? Just the left one.}
\item[Doctor] \emph{How long has this been going on?}
\end{dialogue}
\z
...
Conversations don't just transmit knowledge from one person to another; they actively construct it. This is particularly visible in educational settings, but it happens in all conversations. Consider this exchange between friends:

\ea
\begin{dialogue}
\item[A] \emph{Did you see what happened at the meeting?}
\item[B] \emph{When Sarah walked out?}
\item[A] \emph{Yeah! I couldn't believe it.}
\item[B] \emph{I know. But honestly? I get it.}
\item[A] \emph{You do?}
\item[B] \emph{They've been dismissing her ideas for months.}
\item[A] \emph{Huh. I hadn't thought about it that way.}
\item[B] \emph{Remember the budget proposal she made?}
\item[A] \emph{Oh right. They barely let her finish.}
\item[B] \emph{Exactly.}
\end{dialogue}
\z
...
\subsection{Side sequences} \label{subsec:side-seq}
...
\ea
\begin{dialogue}
\item[A] \emph{So I told Jennifer about the-- }
\item[B] \emph{Wait, Jennifer Chen or Jennifer from accounting?}
\item[A] \emph{Chen. The one who organized the conference?}
\item[B] \emph{Got it. Sorry, go on.}
\item[A] \emph{So I told her about the deadline change...}
\end{dialogue}
\z
B's clarification request suspends A's telling to ensure they're thinking of the same person. Once this is resolved, A resumes the main sequence. Side sequences like this one are closely related to the repair mechanisms we saw in Section~\ref{sec:common-ground}. The difference is structural: a side sequence temporarily suspends the main action to resolve something, then hands the floor back.
\is{action sequence|)}
...
The concept of \enquote{face}, our public self-image and the respect we claim from others, profoundly influences conversational interaction (see Section~\ref{sec:politeness}). As \citet{brown1987} demonstrate, much of what we call politeness involves managing face concerns: both protecting our own face and attending to others'.

Consider how differently we might make the same request:

\ea
\ea Close the window.
\ex Could you close the window?
\ex Would you mind closing the window?
\ex I'm sorry, but would it be possible to close the window? It's getting a bit cold in here.
\z\z
...
This constant, often unconscious, management of face can lead to a significant gap between how a conversation actually unfolds and how we perceive our own performance within it. We are often our own worst critics. In fact, research suggests that we tend to be overly pessimistic about our conversational abilities. After an informal chat, people frequently underestimate how much their partner liked them and enjoyed the interaction, focusing instead on their own perceived stumbles and awkward moments \citep{welker2023pessimistic}. This highlights how deeply the pressure to maintain face is ingrained; we are so focused on monitoring our own performance for potential threats to our self-image that we often miss the signs of success that are clear to
...
\ea
\begin{dialogue}
\item[Manager] \emph{I need that report by Friday.}
\item[Employee] \emph{I'm not sure that's possible. We're still waiting for--}
\item[Manager] \emph{Friday. Make it happen.}
\item[Employee] \emph{I'll... see what I can do.}
\end{dialogue}
\z
...
Many conversations occur within institutional frameworks that shape their structure and possibilities. A classroom discussion, a medical consultation, and a job interview each have characteristic patterns:
...
\ea
\begin{dialogue}
\item[A 2:34] \emph{hey did you see the email about the meeting?}
\item[A 2:34] \emph{apparently they moved it}
\item[B 2:35] \emph{which meeting?}
\item[A 2:35] \emph{to thursday}
\item[B 2:35] \emph{oh the budget one?}
\item[B 2:35] \emph{thursday doesn't work for me}
\item[A 2:36] \emph{yeah the budget meeting}
\item[A 2:36] \emph{ugh that's annoying}
\end{dialogue}
\z
...
\ea
\begin{dialogue}
\item[A] \emph{that's perfect}
\item[A] \emph{(sarcasm)}
\item[B] \emph{\includegraphics[width=0.7cm]{figures/emoji.png} I figured}
\item[B] \emph{sorry!! I'll give it another go}
\end{dialogue}
\z
These repair mechanisms serve multiple functions. The explicit parenthetical \mention{(sarcasm)} clarifies illocutionary force after potential ambiguity, while the emoji response signals both understanding and emotional stance. As \citet{gawne2019emoji} argue, drawing on \citet{dresner2010functions}, emoji function as pragmatic markers of speaker intention: the smiley face in contexts like \enquote{I'm sick and tired all the time :)} does not indicate happiness (emotional interpretation) but rather attempts to soften a statement that might be perceived as a complaint (pragmatic interpretation).
This aligns with their broader framework of emoji as digital gestures that fulfill functions parallel to co-speech gestures, including what they term \enquote{illocutionary} gestures that indicate the type of speech act being performed. Quick follow-up messages, explicit markers, and emoji all serve repair functions that prosody and facial expression handle in spoken conversation, demonstrating how digital communication develops multimodal strategies to restore the contextual cues lost in the transition to text.
\is{emoji|)}\is{digital conversation|)}

\begin{tblsframedsymbol}{Exercise: digital transformations}{pencil}
Examine your recent digital conversations:
\begin{enumerate}[noitemsep, leftmargin=*]
    \item Find an example where message timing created confusion. How was coherence restored?
    \item Identify three strategies you use to compensate for absent prosody/gesture in text. Are these conscious choices?
    \item Compare the same topic discussed in person versus via text. What conversational work becomes visible in the digital version that was invisible in speech?
\end{enumerate}
Consider: Are these limitations of digital conversation, or do they reveal something about how all conversation works?
\end{tblsframedsymbol}

\begin{tblsframedsymbol}{Quick self-check: conversation analysis skills}{test}
Test your understanding of conversational patterns:
\begin{itemize}[noitemsep, leftmargin=*]
    \item Can you identify when overlapping speech signals engagement versus rudeness in context?
    \item Do you recognize how gestures and spatial positioning contribute to meaning beyond words?
    \item Can you spot collaborative completions and repair sequences in natural conversation?
    \item Do you notice how digital communication adapts traditional conversational principles?
\end{itemize}
Developing this analytical awareness enhances both your understanding of conversation as a linguistic phenomenon and your ability to navigate diverse conversational contexts.
\end{tblsframedsymbol}

\section{Summary}

The opening meta-conversation between Jim and Nao demonstrated these principles in action: overlapping speech signalling engagement, collaborative completions building shared understanding, embodied actions carrying meaning, and organic topic transitions emerging from interaction itself, illustrating how conversational patterns operate even under conscious analysis.

\begin{tblsframedsymbol}{Exercise: synthesis: conversation as system}{pencil}
Return to the opening dialogue between Jim and Nao. Using concepts from throughout the chapter:
\begin{enumerate}[noitemsep, leftmargin=*]
    \item Identify examples of at least five different phenomena discussed (turn-taking, repair, embodied action, etc.)
    \item Find one moment where multiple organizational levels interact
    \item Locate an instance where their \enquote{meta-conversation about conversation} demonstrates the very principle they're discussing
\end{enumerate}
Final reflection: How does analysing conversation change your experience of participating in it? Does conscious attention to these patterns interfere with natural interaction, or does it enhance your communicative abilities?
\end{tblsframedsymbol}
