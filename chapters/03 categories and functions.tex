...
\begin{tcolorbox}[title=Exercises, breakable, colback=white]
...
\begin{tcolorbox}[title=Answer key, breakable, colback=white]
...
\begin{tcolorbox}[title=Pause and Reflect, colback=yellow!10!white, colframe=orange!75!black]
...
\begin{tcolorbox}[title=Summary and review, breakable, colback=white]
    ...
    \gap can be modified by adjective phrases. Many \gap can take an \textit{--ly} ending and become an adverbs. Almost every \gap has a third-person singular \textit{--s} form. A/an \gap is often in an agreement relationship with a phrase functioning as its subject. A/an \gap can't be modified by an adjective phrase. The head of a clause is typically a/an \gap phrase.
...
\begin{tcolorbox}[title=From ``The love of a good woman'' by Alice Munro, breakable, colback=white]
...
\item[phrasal category] An syntactic unit, such as noun phrases (NPs), verb phrases (VPs), or adjective phrases (AdjPs), consisting of a head and any dependents.
...
\item[definiteness] A grammatical property that indicates whether a noun phrase refers to something specific and identifiable to both speaker and hearer, typically marked by determiner.
...
\item[attributive modifier] A modifier that appears within an NP -- typically before the hean N -- and attributes some property to it.
