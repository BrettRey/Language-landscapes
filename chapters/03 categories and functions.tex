...
\begin{itemize}[noitemsep]
    ...
    \item Apply diagnostic tests to distinguish adjectives from other word classes
...
\textbf{Step 1 (Easy):} Start with simple noun–noun combinations you know: \textit{soccer ball}, \textit{faculty office}. In each case, which noun is the modifier and which is the head?

\textbf{Step 2 (Medium):} Now try to identify the modifier and head in: \textit{health care}, \textit{phone call}, \textit{college student}

\textbf{Step 3 (Challenging):} Look at the longer chains below and identify the pattern.
\end{tcolorbox}
...
\end{itemize}
\textit{(If you're stuck, think about the word "run" or how both nouns and pronouns can be subjects.)}
\end{tcolorbox}
...
Before doing the exercise below, test yourself:
\begin{itemize}[noitemsep]
    \item Can you define each function type (head, subject, modifier, determiner, complement, object) in your own words?
    \item Do you understand why these are called "functions" rather than "categories"?
    \item Can you give a quick example of each function?
\end{itemize}
\end{tcolorbox}
...
\textbf{Building Complexity Gradually:} As you read through the examples below, start with the simpler, one-word cases first. Then move to the phrase-level examples. Finally, try to identify patterns across different functions. Don't worry if some examples seem challenging at first—this is normal when learning grammatical analysis.
\end{tcolorbox}
...
\textbf{Now You Try:} Before looking at the full passage again, pick 2-3 more words from the passage that you think might be adjectives and test them using the criteria you've learned. Start with words that seem obvious, then try some that are less certain.

\textbf{Reflection Question:} How has working through these examples changed your understanding of what makes a word an adjective?
\end{tcolorbox}
...
\textbf{Think About Your Language Background:} Does your first language (or another language you know) handle possessive pronouns differently than English? How might this affect learning English pronoun agreement?

\textbf{Teaching Tip:} If you're learning or teaching English, these cross-linguistic differences explain why pronoun agreement can be challenging—it's not just about memorizing rules!
\end{tcolorbox}
...
\begin{tcolorbox}[title=Definiteness Self-Assessment, colback=yellow!10!white, colframe=orange!75!black]
\textbf{Test Your Understanding:} Before looking at the tree diagram below, try to explain in your own words:
\begin{itemize}[noitemsep]
    \item What makes an NP definite vs indefinite?
    \item Why might definiteness be challenging for language learners?
    \item Can you give examples of definite and indefinite NPs?
\end{itemize}
\end{tcolorbox}
