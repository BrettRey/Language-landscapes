...
\begin{description}
\item[many-to-many relationship] The principle that grammatical categories and syntactic functions do not have a one-to-one correspondence; a single category can perform multiple functions, and a single function can be performed by multiple categories.

\item[lexical category] A basic word class (noun, verb, adjective, adverb, preposition, determinative) that serves as the foundation for grammatical analysis and phrase structure.

\item[phrasal category] An expanded syntactic unit built around a lexical head, such as noun phrases (NPs), verb phrases (VPs), or adjective phrases (AdjPs), regardless of whether it contains multiple words.

\item[syntactic function] The grammatical role that a word or phrase plays in sentence structure, including head, subject, object, modifier, complement, and determiner functions.

\item[determinative] A word class that specifies the reference of noun phrases, including articles (\emph{the}, \emph{a}), demonstratives (\emph{this}, \emph{that}), and quantifiers (\emph{some}, \emph{many}), distinguished from adjectives by their function and distribution.

\item[determinative phrase (DP)] A phrase headed by a determinative that functions as the determiner in a noun phrase, such as \emph{all of the} in \emph{all of the students}.

\item[definiteness] A grammatical property that indicates whether a noun phrase refers to something specific and identifiable to both speaker and hearer, typically marked by determinatives.

\item[adjective phrase (AdjP)] A phrase headed by an adjective that can function as a modifier within noun phrases or as a predicative complement in clauses.

\item[gradability] The property of adjectives that allows them to be modified by degree words like \emph{very}, \emph{quite}, or \emph{extremely}, and to form comparative and superlative forms.

\item[predicative complement] A phrase that follows linking verbs and provides essential information about the subject, typically filled by adjective phrases or noun phrases.

\item[attributive modifier] A modifier that appears within a noun phrase and directly modifies the head noun, as opposed to predicative uses that appear after linking verbs.

\item[functional overlap] The phenomenon whereby words from different lexical categories can perform similar syntactic functions, creating apparent exceptions to category-based rules.
\end{description}