...
\end{itemize}
(If you're stuck, think about the word \textit{run} or how both nouns and pronouns can be subjects.)
\end{tblsframedsymbol}
...
I will often shorten \textsc{Determiner} to Det. \textsc{Determin\emph{er}} and \textsc{modifi\emph{er}} are syntactic functions. The \textit{--er} tells you it's a function. In contrast, \textsc{Determina\emph{tive}} and \textsc{adjec\emph{tive}} are lexical categories.
...
\begin{itemize}[noitemsep]
    ...
    \item If a phrase is indifferent about a head, it's not a complement. For example, \textit{on Monday} can go with almost any verb \\(e.g., \textit{eat}/\textit{play}/\textit{work}/\textit{think}/\textit{sit \emph{on Monday}}).
...
\begin{enumerate}[noitemsep]
    ...
    \item \is{adjective, adjective phrase (AdjP)!formation of adverb from (-\textit{ly})}Many adjectives can take an \textit{--ly} ending and become adverbs.
\end{enumerate}
...
\begin{itemize}[noitemsep]
    \item \emph{\textit{civil}} \textit{rights}, \textit{some \emph{quite good} news},  \emph{\textit{national}} \textit{security}, \textit{the \emph{whole} thing}, \emph{\textit{recent}} \textit{years}, \textit{our \emph{mental} health}, \textit{her \emph{very best} friend}, \textit{a \emph{really big} deal}, \textit{a \emph{supremely important} responsibility}
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item \textit{bigger}, \textit{biggest}; \textit{greater}, \textit{greatest}; \textit{older}, \textit{oldest}; \textit{lower}, \textit{lowest}; \textit{younger}, \textit{youngest}
...
\begin{itemize}[noitemsep]
    ...
    \item There is no \textit{more}/\textit{most}/\textit{less}/\mention{least following}.
...
\end{itemize}
Therefore, \mention{following} is probably not an adjective, or at best it is a very bad example of an adjective. Since it ends in \textit{--ing} consider whether it might be a verb.
...
\begin{itemize}[noitemsep]
    ...
    \item It is not gradable with \textit{--er}, \mention{more}, or \mention{less}.
...
\textbf{Pattern Recognition:} Notice how we've now seen two words (\textit{following} and \textit{those}) that seem like they might be adjectives but fail the diagnostic tests. What does this tell us about the importance of using multiple tests rather than relying on just one criterion?
\end{tblsframedsymbol}
...
\begin{itemize}[noitemsep]
    ...
    \item There isn't a word \mention{*magnanimouser}, but we do have \textit{more} or \textit{less magnanimous}.
...
\begin{tblslineshorizontal}{Summary and review}
    ...
    \gap It can typically follow \textit{the} or \textit{a}. A word that is countable is a/an \gap.
    Almost every \gap can follow the infinitive marker \textit{to}.
    A noun phrase is typically headed by a/an \gap.
    An adjective phrase is always headed by a/an \gap.
    \gap phrases can perform a variety of functions, including subject and object. Any phrase that can function as complement to \textit{be}, and \textit{become}, \textbf{and} \textit{seem} is \\a/an \gap phrase. Almost every \gap has an \textit{--ing} form.
    \gap commonly denote actions and states, and they do not denote physical objects.

    \phantom{zzz}Many \gap can take \textit{--er} and \textit{--est} endings, but so can many adverbs.
    ...
    \gap can be modified by adjective phrases. Many \gap can take an \textit{--ly} ending and become adverbs. Almost every \gap has a third-person singular \textit{--s} form. A/an \gap is often in an agreement relationship with a phrase functioning as its subject. A/an \gap can't be modified by an adjective phrase. The head of a clause is typically a/an \gap phrase.
    \gap can denote almost anything from objects to actions to characteristics.

    \phantom{zzz}Any word that can be modified by \textit{very}, \textit{too}, and \textit{so} is a/an \gap (or an adverb).
    \gap can't typically be modified by adverb phrases. A word that has a past-tense form is a/an \gap. Many \gap can be gradable with \textit{less} and \textit{least}, and so can many adverbs.
...
There is a good deal of confusion about the terms \textit{determiner} and \textit{determinative}. The former is a function and the latter a category. Unfortunately, some books use them the other way around.\footnote{Those using them flipped around include \textit{A comprehensive grammar of English}, English and Simple English \textit{Wiktionary}, and \textit{Wikipedia}.} I will use  \textit{determin\emph{er}} as a function (like  \textit{modifi\emph{er}}) and \textit{determina\emph{tive}} as a category (like \textit{adjec\emph{tive}}). In general, a reliable dictionary for checking whether a word is a determinative is the \textit{Simple English Wiktionary},\footnote{\url{https://simple.wiktionary.org/wiki/Category:Determiners}} although, confusingly, it calls them ``determiners''. It will help if you can keep the following terminological reminder in mind.

\begin{itemize}[noitemsep]
    \item \textsc{Determin\emph{er}} and \textsc{modifi\emph{er}} are syntactic functions. The \textit{--er} tells you it's a function.
...
If you look back at the Munro passage, you'll notice that most of the determiners you underlined aren't determinatives. They're possessive NPs: \textit{their}, \textit{her}, \textit{his}, \textit{Kath's}. This is the other major category that fills the determiner slot.
