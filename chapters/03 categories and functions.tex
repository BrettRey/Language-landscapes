...
\section*{Orientation and core claims}\label{sec:cat-func-orientation}
This chapter keeps two things separate: \textsc{categories} are word classes like \textit{determinative}, \textit{noun}, \textit{adjective}, \textit{preposition}; \textsc{functions} are roles like \textit{Subject}, \textit{Object}, \textit{Modifier}, \textit{Head}. The same category can serve different functions, and the same function can be realized by different categories. When in doubt, test the category by distribution and internal selection, not by the function it happens to fill here. \is{category}\is{function}\is{Det}\is{Head}\is{Modifier}\is{Subject}

\subsection*{Core claims}\label{subsec:cat-func-claims}
\begin{enumerate}
  \item Category–function independence: no function uniquely identifies a category, and no category is tied to a single function. \is{category!vs.\ function}
  \item NP has a single Det slot. Genitives (\textit{Kim's}, \textit{my}) and determinatives (\textit{this}, \textit{those}, \textit{no}, \textit{each}) are alternatives for the same function; limited stacking (\textit{all the}, \textit{both these}) is governed by structure, not free mixing. \is{Det}\is{genitive}\is{determinative}
  \item Prepositions form one category whether they take an NP complement or not. \textit{They stood inside the house} and \textit{They stood inside} involve the same category: preposition. Modifier selection (\textit{right}/\textit{straight}/\textit{far}) matches this. \is{preposition!intransitive}
  \item Fused heads in NP (\textit{the young}, \textit{the absurd}) preserve the category of the fused item; they don't motivate new categories. \is{fused head}
  \item \textit{More}/\textit{less} are determinatives in NP (\textit{more people}, \textit{less time}). In AdjP/AdvP they function as degree modifiers inside those phrases, not ``as adverbs''. \is{determinative}\is{degree}
\end{enumerate}

\subsection*{Tests used in this chapter}\label{subsec:cat-func-tests}
\begin{enumerate}
  \item \textbf{Substitution within category.} Swap D for D (\textit{those} → \textit{these} → \textit{no}); swap P for P even when no NP follows (\textit{inside} → \textit{outside}). \is{substitution test}
  \item \textbf{Internal selection.} \textit{very} selects AP/AdvP and not P (\ast\textit{very inside}); \textit{right/straight/far} select P (\textit{right up}, \textit{far inside}). These pairings diagnose category. \is{modifier selection}
  \item \textbf{Complement profile.} When present, an NP after P (\textit{up the ladder}) is decisive. When absent, keep the category if operator selection and distribution match P. \is{complement selection}
  \item \textbf{Det cardinality.} One Det per NP, with restricted pre‑Det quantifiers (\textit{all/both}). Genitives occupy the same slot as determinatives. \is{Det!cardinality}
  \item \textbf{Fusion check.} In fused‑Head NPs (\textit{the poor}), the fused item behaves like the NP Head externally while keeping its own category internally. \is{fusion}
\end{enumerate}
...
This chapter's primary goal was to establish a clear and consistent distinction between linguistic \textbf{categories} (word classes) and \textbf{functions} (grammatical roles). The key takeaways are:

\begin{itemize}[noitemsep]
    \item \textbf{The Core Principle:} A word's category (noun, preposition, etc.) is determined by its formal properties---what it combines with and how it behaves---not by the function (Subject, Modifier, etc.) it happens to perform in a sentence. There is a many-to-many relationship between categories and functions.

    \item \textbf{Key Analytical Tests:} This principle was applied using a small set of tests, leading to several important clarifications:
    \begin{itemize}[noitemsep]
        \item An NP has only \textbf{one Determiner slot}, which can be filled by a determinative (\textit{the}, \textit{each}) or a genitive phrase (\textit{Kim's}, \textit{my}), but not both.
        \item Words like \textit{inside}, \textit{up}, and \textit{down} belong to a single \textbf{preposition} category, whether they are followed by an NP complement or not. This is confirmed by their ability to be modified by words like \textit{right} and \textit{straight}.
        \item \textbf{Fused heads} in NPs (e.g., \textit{the young}) do not create a new word class; the headword retains its original category.
    \end{itemize}

    \item \textbf{Revisiting Word Classes:} Using these methods, we distinguished \textbf{determinatives} from adjectives and pronouns and defined the core properties of \textbf{adjectives}, \textbf{pronouns}, and their related phrases (AdjP, DP).

    \item \textbf{Syntactic Functions:} The chapter also defined the foundational syntactic functions: Head, Dependent, Subject, Complement, Object, Modifier, and Determiner.
\end{itemize}

Teachers should be prepared to explain these concepts clearly and simply, but not simplistically. Establishing a solid foundation that separates category from function prevents confusion and avoids setting traps that students might fall into later.