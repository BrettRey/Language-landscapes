...
Of course, this can't be right. A high school is a place (or a thing?), which should make \textit{high school} a noun under the definition above, but in the example sentence, it's supposed to be an adjective. In other cases, such as \textit{the Trudeau government}, Trudeau is a person but the word is, in the phrasing of \textit{The blue book of grammar and punctuation}, \enquote{modifying the noun} \textit{government}. So is \textit{Trudeau} a noun or an adjective? Or is it in some kind of quantum superposition of being both at the same time? In fact, both \textit{high school} and \textit{Trudeau} are just nouns, full stop (check the properties in Section \ref{sec:nouns} yourself if you doubt this).
...
\begin{itemize}[noitemsep]
    ...
    \item Complements are \enquote{selected} by a head \\(e.g., \textit{listen \emph{to the music}} not \textit{*listen \emph{about the music}}; \textit{listen} selects a \textit{to} PP.).
...
\begin{itemize}[noitemsep]
    ...
    \item We can't say something is \textit{the followingest}.
    \item There is no adverb \textit{followingly}.
    \item It can't function as the complement of \textit{become} or \textit{seem} \\(e.g., \textit{the day became following}).
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item It can't be modified by \textit{very}.
    \item It can't function as complement of \textit{become} or \textit{seem}.
...
\end{itemize}
So, \textit{magnanimous} seems to be an adjective.
...
\is{pronoun!defined}Pronouns are a special kind of noun. They typically refer to another noun phrase that has come before or that comes later (e.g., in \textit{Xi saw \emph{the egg} fall but caught \emph{it} before \emph{it} smashed}, the pronoun \textit{it} refers to the egg). There are only a small number of pronouns. The main ones are set out in Table \ref{tab:pronouns}.
...
A \is{dummy pronoun!defined}\textsc{dummy pronoun} is one which does not refer, as in \textit{\emph{There} are two dummy pronouns} or \textit{\emph{It}'s windy today.}\label{sec:dummy} There is no semantic reason for \textit{there} and \textit{it} to appear in those sentences. Its role is purely grammatical, because English demands a subject in this context.
...
In traditional English grammar, there is no concept of determiner or determinative. There was a special category for \textit{the} and \textit{a} called articles, but all the other determinatives were just called \enquote{adjectives} when they preceded a noun. For instance, the word \textit{some} in \textit{some people} is categorized as an adjective in the \textit{Merriam-Webster dictionary}, even though it has none of the other characteristics of adjectives. There are two reasonably good ways to distinguish whether a word is a determinative or an adjective.
...
Like pronouns, determinatives can often refer to NPs that appear elsewhere in the discourse (e.g., \textit{I bought \emph{a bag of chocolate chip cookies}, and I ate \emph{some} on the way home}). For this reason, they are often categorized as pronouns. For example, this use of the word \textit{some} is categorized as a pronoun in the \textit{Merriam-Webster dictionary}, even though it has none of the other characteristics of pronouns. For one thing, \is{determinative, determinative
...
But there's a subtlety here. Determiner choice is a grammatical fact about the NP. Whether the speaker expects the listener to identify the referent is a semantic one. The two usually align, but not always~-- generic \textit{the} (\textit{the lion is a noble beast}) is grammatically definite but doesn't pick out a specific lion, while specific \textit{a} (\textit{a certain student kept disrupting class}) is grammatically indefinite but does.
...
The dependent genitive pronouns in Table \ref{tab:pronouns}~-- \textit{my}, \textit{your}, \textit{his}, \textit{her}, \textit{its}, \textit{our}, \textit{their}~-- are all NPs. They're nouns (specifically pronouns), and they function as determiners, just as \textit{the} and \textit{some} do. The difference is purely categorical: \textit{the} is a determinative; \textit{my} is a pronoun.

Longer possessive NPs work the same way. In \textit{my mother's sister}, the NP \textit{my mother's} functions as determiner in the larger NP. And \textit{my mother's} has its own internal structure: the pronoun \textit{my} functions as determiner inside it. So we have an NP nested inside an NP, each with its own determiner:

\begin{itemize}[noitemsep]
    \item \textit{[\emph{my} mother's] sister}~-- \textit{my} is Det in \textit{my mother's}; \textit{my mother's} is Det in \textit{my mother's sister}
    \item \textit{[\emph{the} dog's] owner}~-- \textit{the} is Det in \textit{the dog's}; \textit{the dog's} is Det in \textit{the dog's owner}
    \item \textit{[\emph{this} morning's] headlines}~-- \textit{this} is Det in \textit{this morning's}; \textit{this morning's} is Det in \textit{this morning's headlines}
\end{itemize}
...
Notice how this connects to the many-to-many theme from the start of the chapter. The determiner function can be filled by a DP (\textit{the}, \textit{some}, \textit{every}) or by a possessive NP (\textit{my}, \textit{Brett's}, \textit{the dog's}). Two different categories, one function.
