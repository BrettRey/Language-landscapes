...
\begin{tcolorbox}[title=Practice: Analyzing Conversational Implicatures, colback=yellow!10!white, colframe=orange!75!black]
...
The maxims help us recognize when additional meaning might be present, but they don't explain how we actually arrive at specific interpretations. When someone says \textit{the popcorn was good} in response to a question about a movie, how do we leap from this apparent non-sequitur to ``the movie was bad''? The answer lies in \textsc{abduction}~-- inference to the best explanation.
...
\begin{tcolorbox}[title=Recognizing Speech Acts in Context, colback=yellow!10!white, colframe=orange!75!black]
...
\begin{tcolorbox}[title=Practice: Analyzing Speech Acts, breakable, colback=white, parbox]
...
\begin{tcolorbox}[title=Cross-cultural Pragmatic Failure, breakable, colback=white]
...
This chapter explored pragmatics as the study of how context shapes meaning beyond literal semantics, examining the crucial role of inference and shared assumptions in communication:

\begin{itemize}[noitemsep]
    \item \textbf{Pragmatics vs. semantics}: While semantics deals with conventional word meanings, pragmatics examines how context, shared knowledge, and communicative principles create meaning in actual use
    \item \textbf{Cooperative Principle}: Grice's maxims (Quality, Quantity, Relation, Manner) describe normal conversational expectations and explain how violations create conversational implicatures
    \item \textbf{Speech act theory}: All utterances perform actions through locutionary acts (producing words), illocutionary acts (intended functions), and perlocutionary acts (achieved effects)
    \item \textbf{Politeness strategies}: In communication, we have to manage positive face (desire for approval) and negative face (desire for autonomy) through various mitigation strategies
    \item \textbf{Context sensitivity}: Physical, social, linguistic, and cognitive contexts all shape interpretation, requiring speakers and hearers to calibrate meaning continuously
    \item \textbf{Common ground}: Successful communication depends on shared knowledge, beliefs, and assumptions between interlocutors
    \item \textbf{Cross-cultural variation}: Pragmatic norms vary significantly across languages and cultures, affecting directness, politeness, and appropriateness judgments
    \item \textbf{Pragmatic competence}: Effective language use requires understanding not just grammar and vocabulary but also contextual appropriateness and cultural expectations
    \item \textbf{Teaching implications}: Pragmatic instruction benefits from explicit discussion, authentic materials, cross-cultural comparison, and safe practice environments
\end{itemize}