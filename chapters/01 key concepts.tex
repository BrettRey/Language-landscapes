...
\begin{tcolorbox}[float*, title=A thinking trap, breakable, colback=white]
...
\begin{tcolorbox}[title=Questions, breakable, colback=white]
...
\begin{tcolorbox}[title=Answers, breakable, colback=white]
...
\begin{tcolorbox}[title=Find the nouns, breakable, colback=white]
...
\newpage
\begin{tcolorbox}[title=Why does this matter?, breakable, colback=white]
...
\ex[]{\gll \emph{Картошка} \emph{была} \emph{вкусной}.\\
    potato is hot\\
    \glt `The potatoes are hot.'}\label{ex:Картошка}
\ex[]{\gll\emph{ekṭa} \emph{khobor} \emph{āche}\\
    a news is/exists\\
    \glt `There's some news.'}\label{ex:khobor}
\ex[]{\gll\emph{Þe} \emph{pease} \emph{wæs} \emph{swete}.\\
...
There is great diversity in the world. It is important for us not be so blinded by our own intuitions as to believe that our way of categorizing stuff into that which can be counted and that which can't is in any way natural or objective. And remember that when your students make the Standard English choice, you should respect them for having achieved an insight into the peculiar way of looking at the world shared by English speakers.\is{countability (count vs non-count noun)|)}
...
\ea \label{ex:defined}
    \ea[*]{one cats/things/students/studies/women/mice\hfill [definition works]}
    \ex[*]{minus one degrees/points/months\hfill [definition works]}
    \ex[]{minus two degrees/points/months\hfill [unclear: $-2 < 1$ but $|-2| > 1$]}
    \ex[]{zero degrees/points/months\hfill [definition fails]}\is{plural!and zero}
    \ex[]{0.5 degrees/points/months\hfill [definition fails]}
    \ex[]{1.0 degrees/points/months\hfill [definition fails]}
    \z
\z
...
\ea \label{ex:no}
    \ea[]{no way/idea/problem}\label{ex:sing-no}
    \ex[]{no ways/ideas/problems}\label{ex:plur-no}
    \z 
\z
...
\ea \label{ex:undefined}
    \ea[*]{one furniture/pottery/footwear\hfill [undefined]}\label{ex:sing-non-count}
    \ex[*]{one jeans/glasses/police\hfill [undefined]}\label{ex:plur-non-count}
    \z 
\z
...
\begin{tcolorbox}[title=Practice, breakable, colback=white]
...
\begin{description}
   ...
   \item[noun, non-count] A non-count noun is one that can't be counted (e.g., *\textit{1 furniture}, *\textit{2 groceries}). See Section \ref{sec:count}.
...
\begin{tcolorbox}[title=Short Answer Quiz, breakable, colback=white]
...
\begin{tcolorbox}[title=Answer Key, breakable, colback=white, colframe=black, coltitle=black, fonttitle=\bfseries, colbacktitle=gray!10!white]
...
\begin{tcolorbox}[title=Essay Questions, breakable, colback=white, colframe=black, coltitle=black, fonttitle=\bfseries, colbacktitle=gray!10!white]
